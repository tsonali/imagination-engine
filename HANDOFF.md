...
_Last updated 2026-08-28 beat197 (sonali-52 session) — **SHIP GATE HOLDS. Closed beat196's highest-priority open finding: traced the VF fabrication ("without the kids around") to its structural root cause. `vital_facts.context_block()`'s raw content is injected on EVERY turn via `_running_context()` (this includes the second-pass forced-response fallback path, since `ctx` feeds `user` feeds `user_fwd`), but the only instruction grounding `## People` facts as belonging to a named OTHER person, not the user (`_vf_probe_supplement()`), fires ONLY on detected memory-probe turns. An ordinary conversational turn gets the raw "Sister: Priya — Austin, two kids" block with zero attribution guardrail, on any path — the gap was never path-specific scrutiny (beat196's hypothesis), it was that the grounding instruction's scope (probe-only) didn't match the injection's scope (every turn). FIXED in vital_facts.py: moved a standing attribution rule into `context_block()`'s own header, present on every turn/path by construction, same "closes every path at once" pattern as beat139's software-pronoun guard and beat196's own date-tag strip. TWO more mechanical fixes: (1) new pronoun-grammar class — bare "you" standing in for "your" before left/right + body part ("toward you left hip" / "on you left wing tip" / "with you left wing"), confirmed across 3 independent scenarios (imag-mri, imag-eagle-wildlife-plural, imag-eagle-companion-bird-he); new `fix_you_before_bodypart()` in postcheck.py, wired into both generator.py pipeline call sites. (2) eagle acoustic anon-companion escape "the call of the distant eagle" (imag-embodiment-eagle) — species named directly, unlike the already-covered "distant bird"; added to all 3 places this pattern family is independently duplicated (postcheck.py, generator.py, battery11_imagination_bank.py — 3-way parity). All fixes verified via py_compile + `scripts/test_postcheck.py` (ALL PASS) + direct unit tests against exact defect strings + FP checks — no model launch (memory 8-27% free the entire beat, qc_queue's own `battery9_engagement.py` running throughout, ~70+ min in flight at close). vital_facts.py MD5: 14aa24c27afd1cfd9a2976335b70c29b. postcheck.py MD5: c9aec5e0fbbccdfbb57f0ab3772bf78d. generator.py MD5: 0ad4c74ee0377c1233eae37b4b451d35. battery11_imagination_bank.py MD5: 5f6ba82c7cbc77965ef84061b7d08e38. All dist copies synced (verified each packaged file's MD5 matches src post-rebuild). ZIP REBUILT: dist/hearth-0.2.zip MD5 f7811a8877963634c19a913dd52faa19. Read 6 small backlog logs directly (all clean) + `battery11_0828_0818` via a background agent (7 scenarios, found the 3 fixes above plus 5 lower-confidence FYIs: truncated-artifact fragment, severe back-half decay, "particular"/"specific" crutch phrase reconfirmed in imag-calm-settle/imag-mri — escalates beat196's finding further, 2 minor decay pairs, 1 typo). Directly checked beat196's escalated 2nd→3rd-person narrator-drift finding against this log: zero instances — does not retire the finding (beat196 confirmed 15+ hits elsewhere), still wants the dedicated design pass. No gold added this beat (full time went to the structural VF fix); gold corpus grew 6618→6627 from peer sessions, not duplicated. Mini still unreachable — same underlying DNS-alias-resolution-failure signature (`~/.ssh/config`'s `mac-mini.localdomain` → `julios-mac-mini.local` alias fails), not a new data point, 71st+ consecutive beat. Coordination: 6 peer sessions active on arrival, claimed an unclaimed log slice, no collision. Sonali: push v1.0 tag (git push origin v1.0). Only Sonali-physical: notarization + F5 voice dial.**
