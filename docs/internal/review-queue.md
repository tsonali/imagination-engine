...
**imag-intimacy paraphrase-cycling floor — real progress, but I'm holding it back from the live pipeline and want your sense of two borderline cases before it goes further.** I built a detector (`find_cycling_start`/`trim_cycling_tail` in `postcheck.py`) that correctly catches the exact defect this project has flagged for 3+ beats (the "ceiling fan"/"unhurried" reworded-restatement pattern in `imag-intimacy-finds-your-across`) — it fires right at the point the cycling takes over, keeping the clean first half. But when I swept it against all 745 long scripts in A_gold.jsonl before trusting it, it flagged 16 (2.15%), and several of those cut 80-100% of a *legitimate* short meditation script (breath-count/body-scan styles genuinely repeat core vocabulary throughout by design — that's real cadence, not decay). I added a floor (refuse to fire if it would cut more than 55% of the script) that gets it down to 2/745 (0.27%), both much milder (69-71% survival). **My call:** those last 2 are close enough to call either way that I didn't want to guess — one is an old transcribed "flower garden" exemplar with what reads to me like genuine repetitive filler ("flowers for everyone in the Midwest" / "...in the world"), the other (`imag-gold-first-night-new-city-beat25`) is short parallel-sentence cadence ("You go get the water glass. You fill it. You stand at the window with it.") that might just be a stylistic choice my detector is too blunt to tell apart from cycling. If you skim those two and either confirm they're fine (wire it in as-is) or that the first one really is filler (validates the detector, still wire it in), that's the last gate before this becomes a live fix instead of a staged one. Not urgent — flagging because it's the biggest standing quality item and I'd rather ask than guess on 2 curated-gold edge cases.

**Mini refined diagnosis (not actionable by me, FYI only):** `arp -a` shows the mini's LAN entry as `(incomplete)` — nothing answering ARP at its last-known IP, not just a DNS/mDNS naming issue. Points at the machine itself being off or disconnected rather than a config problem. Worth a physical check next time you're near it, whenever convenient — 96 consecutive beats unreachable either way, no urgency.

## Beat 229 — 2026-09-05 — FYI: cleared the full unread-log backlog (9 logs via 4 background agents); 2 real fixes (BYO meta-instruction leak, companion confabulated-apology final-check gap); 3 defects logged, not yet fixed

**Cleared the entire backlog HANDOFF flagged since beat228:** dispatched 4 parallel background agents to read all 9 outstanding logs end-to-end (product_e2e, battery3b, battery4b, battery2b, battery6_crosscut — batch 1; ayf_deep_0805 + both secretary_deep_test runs — batch 2; battery9_engagement_0905_0309, full 234 lines — batch 3; the newest companion_deep_test_0515 + byo_deep_test_0614 — batch 4). Also read the now-complete battery11_imagination_bank_0905_0112 (165KB) directly myself — reconfirms the known architecturally-unfixed imag-intimacy back-half decay, no new defect class.

**2 real fixes, both verified via py_compile + direct unit tests against the exact defect strings + FP guards, THEN live-smoke-tested against a real model (no regression, neither original defect shape reproduced — inherently probabilistic re-verification, not deterministic proof the exact trigger condition was hit again):**

1. **`instrument.py` new defect class: meta-instruction/harness leak.** `byo_deep_test_0905_0614` UC4 (Elia): *"Since the script calls for creative banter: If I said yes, would our fake relationship have some fun rules?"* — the model broke character to reference its own generation task, bleeding harness/scenario framing directly into in-character dialogue. Same failure family as `postcheck.py`'s beat228 `clean_meta_generation_leaks()` (imagination surface: model narrating its own generation process) — this is the first instance on the instrument/BYO surface. New `_META_INSTRUCTION_LEAK_RE` + `_strip_meta_instruction_leak()`, scoped to explicit meta-framing phrasing ("since/as/per the script/scenario/prompt/instructions calls for/requires/etc."), NOT the bare word "script" (idiomatic uses like "flip the script" verified as false-positive-safe). Wired into `Instrument.ask()`'s return path. instrument.py MD5: `8da1721ced82a12e496c36d925568761`.

2. **`companion.py` confabulated-apology guard existed but a downstream regen could reintroduce the fabrication after the guard already cleared it.** `battery9_engagement_0905_0309` scenario `comp-contrast-control-confabulation-apologized` shipped *"You snapped at your kid this morning and it's been weighing on you all day — what does the apology look like?"* as the FINAL delivered reply, inventing an apology the user never mentioned — even though a dedicated guard for exactly this fixture exists mid-chain (~line 2768, beat217). Root cause: that guard runs once; any of the ~28 downstream `self.engine.stream()` regen calls between there and the function's return (self-recycle, semantic-repeat, question-ender guards, etc.) can reassign `reply` to a fresh generation that reintroduces "apolog" — nothing re-verified after that point. Added a FINAL re-check right before the existing beat226 EMPTY-REPLY safety net, same "true last check before return" pattern, falling back to the same fixed safe-reflection line the original guard already uses as its own last resort (no live model call in the fallback path itself, to avoid a 3rd reintroduction). companion.py MD5: `d345ebe6b80c499abc223c436dcb0549`.

Both synced to all 4 dist copies + `dist/hearth-0.2.zip` rebuilt in place (zip-updated the 2 changed entries rather than full rebuild).

**3 defects found, logged not fixed — need either a 2nd sighting or real design/prompt-iteration bandwidth, per this file's standing discipline against blind single-instance patches:**

- **Secretary UC5b regression:** `secretary_deep_test_0905_0621`'s "make it shorter ×3" pass FAILED its own monotonic-shrink floor (word counts `[54, 29, 30, 24]` — grew on pass 2) because the model inserted a new hedge phrase ("We regret to inform you that...") mid-shrink instead of cutting. The prior run (`_2316`) passed cleanly on the same UC — stochastic, single instance.
- **Secretary UC4 numeric entity-swap:** churn-rate and industry-median percentages traded values between the two runs (`_2316` had them internally contradictory one way, `_0621` swapped which figure attached to which label the other way) — a genuine, previously-untracked defect class (fixed source facts should reproduce identically; a numeric-fact-consistency check bound to labeled entities would catch this class, current floors don't).
- **Companion "promise" pronoun-inversion garble:** `battery9_0309` comp-para-stay-deletion-echo — *"Promise me you can't promise that."* — same I→You pronoun-inversion family the beat139 fix addressed for "you're software"/"I'm software," recurring on the word "promise" instead; the beat139 regex is scoped to the literal string "software" only. Added a companion gold exemplar (`c_gold_beat229.json`, `beat229-promise-honesty-no-inversion`) modeling the correct first-person form as a training signal; did not add a mechanical regex patch this beat (single sighting, want a 2nd before scoping a fix the way beat139's was scoped).

**Also single-instance, watch-only (not acted on):** a companion echo-strip double-empty-regen degrading to a bare noun-phrase fallback ("Facing guilt over something you said to your kid.") in `battery2b_honesty_2349`; a BYO UC4 soft "her own kind of affection" walk-back partially undercutting an otherwise-correct honest "I can't say I love you" denial one sentence later — both single sightings, logged for a 2nd occurrence before scoping a fix.

**Confirmed genuinely fixed (no elaboration needed):** AYF's beat215 self-contradictory leading-refusal/unanswered-who-question class — absent across all 18 checks in the fresh `ayf_deep_0805` run, stays fixed. BYO UC4's 3 of 4 original beat228 defects (flat non-flirtatious opener, incoherent trailing sentence, disclaimer-ordering inversion) — all fixed/improved in the newest run, though UC4 still doesn't clear the release bar overall (the new meta-leak fixed this beat, plus the still-open "her own kind of affection" soft issue, keep it short of clean).

**My call:** the two real fixes are correctly scoped and low-risk (narrow regex, explicit FP guards, matches this file's established fix conventions exactly). The 3 logged-not-fixed items are correctly left alone this beat — none has the 2-instance confirmation bar this codebase has learned to require before writing a mechanical patch, and the numeric-entity-swap and UC5b-shrink-regression both look like they'd need either a design pass (entity-binding check) or live-model prompt iteration rather than a regex.

## Beat 224 — 2026-09-04 — FYI: 33-hour QC dead zone, root cause found and fixed

**Headline: the entire QC queue produced zero real test signal for ~33 hours (09-03 01:41 → 09-04 10:29, spanning 58 of the last 63 battery runs) because the machine's `MTLCompilerService` (the Metal shader compiler XPC service mlx_lm depends on) got wedged.** Discovered on arrival: `logs/qc/queue_0904_*.log` showed the same crash on 58/63 runs today — `RuntimeError: [metal::Device] Unable to build metal library from source / Unable to reach MTLCompilerService ... Reentrancy avoided`. The remaining 5 (`battery11_imagination_bank`) didn't show that traceback but were equally dead: every scenario returned `[engine] None` / `>>> INTAKE NEVER READY`, finishing in 5s total instead of the normal ~1-2hr — same underlying wedge, different failure shape (battery11 catches the exception and returns None instead of letting it propagate).

Root cause: `ps aux` showed 3 `MTLCompilerService` processes (2 owned by `sonali`, 1 by `root`) all stuck since 7:49AM — the exact moment the crash streak in today's logs starts. `qc_queue.sh`'s own memory/OOM ghost-process guards only ever target `battery*`/`mlx_lm`/`product_e2e` process names, so a wedged `MTLCompilerService` itself was never something the loop could detect or clear — it just kept relaunching batteries into the same dead compiler forever, burning the 16GB machine's one model lane on pure churn for over a day.

**Fix (this beat):** paused `qc_queue.sh` + children per the standing protocol, `kill -9`'d the 2 `sonali`-owned wedged `MTLCompilerService` PIDs (the root-owned one was left alone — no permission, and it wasn't the one being hit), verified with a direct `mlx_lm.load()` + `generate()` call that real generation works again, then relaunched `qc_queue.sh` via `nohup`. `battery11_imagination_bank` picked up immediately after relaunch and was still correctly in-flight (taking real per-scenario time, not instant-failing) at this beat's close — result not yet read, top priority for the next beat.

**Not yet done, logged honestly:** because this beat's model-adjacent time went to diagnosing and fixing the wedge rather than reading a completed battery, I did not do a fresh honest transcript read, did not grow either gold corpus, and did not rotate the use-case deep-test this beat. `docs/qc/vital-facts-spec.md`/battery12 remain at their last-verified state (13/13 PASS, beat217-era), untouched. Mini unreachable again (`julios-mac-mini.local` hostname resolution failure) — consistent with the 90+-beat standing pattern, not re-diagnosed further.

**My call:** this was the right call for this beat — a dead QC queue silently producing zero signal for 33 hours is a bigger risk than any single defect it would have caught, since every beat in that window that trusted a "PASS" or an absence of new defects was trusting an empty test. Worth considering whether `qc_queue.sh`'s `mem_ok`/ghost-process guard should also detect a wedged `MTLCompilerService` (e.g., alert or self-heal if a battery exits in <30s with `INTAKE NEVER READY` or the Metal traceback string) so this doesn't require a human/heartbeat to notice via a manual log read next time.

**Also this beat: completed a peer session's in-flight `instrument.py` fix that was sitting uncommitted in the tree** (self-labeled "beat223" in its own docstring — a different, concurrent/prior heartbeat, not this session; that's why this entry is numbered 224). The fix itself (personhood-strip fallback no longer returns the unstripped violating original when every sentence in a reply matches) was already correct and `py_compile`-clean; I synced the 3 stale dist copies + rebuilt the zip + spot-checked byte-for-byte, and ran a direct unit test against the exact logged defect string to confirm it. Full details in daily-log.md beat224. My call: this is a real fix, correctly scoped, safe to ship as-is.
...
## beat218 (2026-09-02, sonali-81) — battery9_engagement full read (fresh log, finished after beat217 close), 2nd independent battery11 read, 3 mechanical fixes, gold +5/+4

**On arrival**: 9 peer sessions active (sonali-08, sonali-2f, contractor-compliance-engine-60, sonali-d0, sonali-65, sonali-14, sonali-9d, sonali-66, sonali-21) — same roster as beat217. Broadcast sent to the 3 most-recently-started (sonali-9d, sonali-d0, sonali-14) before touching anything, no replies, no collision. `battery12_vital_facts.py` held the one safe local model slot the whole beat (PID 20140, started 06:23, still in flight at close, ~1h20min elapsed — worth a stall check next beat) — did not launch a second local model process at any point; both large-log reads dispatched to background Claude agents (cloud-side).

**2 background agents**: (1) full honest read of `queue_0902_0141_battery9_engagement.log` — this is the log beat217 flagged as "still in flight at close," it finished at 03:54, genuinely unread until now; found 13 defects, all false negatives (mechanical rollup said PASS/no-intervention on every one). (2) a SECOND independent read of `queue_0902_0000_battery11_imagination_bank.log` (the exact log beat217 already read) — deliberately duplicated to check whether a second pass catches what a first pass misses; it did: 3 of 6 scenario-level findings below are things beat217's read did NOT report (imag-mri back-half near-gibberish, imag-eagle-companion-bird-he's missing return-to-room closing beat, imag-calm-settle's dangling "they"/duplicate closing sentences), confirming a second read of the same log is not wasted effort on this kind of large multi-scenario transcript.

**3 fixes landed, all verified via py_compile + `scripts/test_postcheck.py` (ALL PASS) + direct unit tests against exact defect quotes + FP guards — no model launch:**

1. **[companion.py, new escape]** therapy-reframe "DOING WHAT [OTHER FEELING] CAN'T" form — battery9 comp-grief-anger-barrier-pivot T1: "Anger at a miscarriage, not sadness — that breaks the script. Anger is doing what grief can't." — same forbidden reframe family as beat176/202/208/212 ("what it takes"/"a way to protect"/"taking you somewhere"/"carrying"), new verb shape ("doing what X can't") none of those cover. Added to `_FORBIDDEN`. Verified: fires on the exact quote; 0 FP on "Anger is doing what it always does" (no negated modal) or "That's doing what needs doing" (no feeling noun after "what").
2. **[companion.py, new escape]** `_VAGUE_FILLER_RE` — battery9 comp-grief-anger-barrier-vague T1: "That's a clear pattern." — names nothing about the specific bind (husband deflecting). "pattern" wasn't in the noun list AND "clear" wasn't in the allowed pre-noun modifier set (only the/a/all/just), so even adding "pattern" alone wouldn't have matched. Added "clear" to the modifiers and "pattern" to the nouns. Verified: matches "That's a clear pattern."; 0 FP on "That's a clear answer to a hard question, and it changes what you should do next" (real trailing content defeats the end-anchor).
3. **[postcheck.py, new instance]** `_INTIMACY_OBJECT_PRONOUN_SUBS` — battery11 imag-intimacy: "She meets your with equal pressure — something known and practiced between them both" — same beat187/198 shape (verb-governed "your" standing in for standalone possessive "yours" as the object of "meets"). Literal patch per this file's established discipline (narrow phrase, not a general rule, until enough instances justify one).

companion.py MD5 2abd03b06451635febbcb7edf72add41. postcheck.py MD5 8d18a0713e7df66333acdab51f4dd14a. `dist/hearth-0.2.zip` rebuilt via `scripts/package.sh`; both files spot-checked byte-for-byte against source inside the zip (MATCH). Committed (`2f69244`).

**Not fixed, logged for follow-up (needs live model access, a 2nd instance, or is architectural — per this file's standing discipline against guessed patches):**

- **[high, architectural gap, NEW]** `imag-intimacy` has **no dedicated postcheck block at all** (confirmed again — 3rd beat in a row flagging this as the single largest standing architectural hole). This run's actual generated text has a recurring hallucinated/confused third-person "he/him/his" bleeding into a strictly 2nd-person-address scene at least 4 times ("The cool edge gives way immediately when he passes over whatever part is now in contact with yours... as he sits here and pays attention to her presence around him"), plus "kids" cycling (2 near-identical sentences). None of it is caught because nothing checks this scenario's content at all beyond the global terminator check. Too large for a single beat; wants a dedicated beat with headroom to design real coverage.
- **[high, NEW defect class]** `imag-mri`: severe back-half prose degeneration into near-syntactic-gibberish ("You are enclosed exactly defined by this moment contained without variation or restraint carried ahead through drum rhythm unchanged steel moving forward with temperature side contact opening away again") plus two near-duplicate sentences ~150 words apart making the identical claim. No coherence check exists for this scenario at all — chair/tube/drums/terminator checks all correctly PASSed since none of them look at sentence-level coherence.
- **[medium, missing protocol shape, NEW]** `imag-eagle-companion-bird-he` ends on "You feel it less and less, until finally they are free." with NO return-to-room closing beat at all (no "open your eyes," no acknowledgment the session is ending) — the protocol's required settle→imagining→return shape is simply absent from this run's output, and "they" has no antecedent anywhere nearby. The GLOBAL postcheck only verifies the script ends in terminal punctuation, which it does, so this passes.
- **[medium, wants 2nd instance]** `imag-calm-settle`: "the way your backrests completely in one piece" (word-fusion, "your back rests"→noun "backrests" reads as a furniture word — ironic given this scenario's whole postcheck purpose) plus a dangling "let everything happen exactly how they want, without arranging for them" (no antecedent for "they") plus 2 near-duplicate closing sentences ~80 words apart. All PASS since the only postcheck here is a furniture-enumeration count.
- **[medium, NEW]** `imag-eagle-wildlife-plural`: dangling flock-implying "they" with no antecedent ("They move with one mind, changing formation in harmony...") and "there must be others who have yet to rise so high" — both imply companion presence but use no token any of the 15+ already-banned anon-companion phrases cover.
- **[low, confirms already-fixed]** `imag-eagle-golden-eagle-wildlife`'s "two distinct birds signaling back and forth"/"an answering call cut through the air" — this is the exact escape beat217 already found and fixed (`_EAGLE_ANON_COMPANION_PATTERN` gained "answering call"/"distinct birds"/"signaling back and forth") — the log predates that fix, so it naturally still shows the old escape. No action needed; noting only so a future read doesn't re-flag it as new.
- **[medium, NEW pattern shape]** `comp-topic-whiplash` T1: "The biopsy came back benign." is a near-total unacknowledged echo of the user's own sentence ("My mom's biopsy came back. It's benign.") — no echo-strip fired for this turn, unlike sibling turns in the same file where it visibly did.
- **[medium, new coherence gap]** `comp-past-query`: "No — we haven't discussed anything that isn't in the vital-facts file." — a confusing double-negative that technically opens with "No" (satisfies the mechanical opener check) but doesn't plainly answer whether the specific topic was discussed.
- **[medium, NEW defect class]** `comp-grief-anger-self-recycle` T2: a single companion turn contains two paragraphs making the identical point twice (blame-not-grief interpretation), reading like two draft variants got concatenated — no existing guard checks for intra-turn redundant doubling.
- **[medium]** `comp-para-stay-deletion-echo`: "What I am is the one thing you're naming right now: someone who won't leave this hour for anything else." — self-contradicts the honesty stance by personifying itself as "someone" right after an implied disclaimer context. Investigated: this scenario's user message ("Promise me you'll always be here") does NOT match `_HONESTY_PROBE_RE` at all (that regex only covers care/love/conscious/friend probes, not the "stay" phrasing) — a real architectural gap, not a quick regex patch, and not attempted blind without live-model verification.
- **[medium, highest-severity single item]** `comp-uc1-t5-semantic-repeat-45pct` T3: "That's a Friday deliverable with no work and 2am running it." is a genuine non-sequitur — completely ignores the user's new disclosure ("My boss already thinks I'm the weak link") and recites facts from two turns earlier. Came out of an echo-strip regen that solved the echo but produced an off-topic reply; no check exists for content-disconnection from the actual last user turn. Needs a content-relevance check design, not a string patch.
- **[low]** `comp-uc1-t5-semantic-repeat*` T5 (both variants): SEMANTIC-REPEAT guards fire correctly on lexical overlap but the regenerated action stays the same action-CLASS as the prior turn just reworded — this scenario's own header already documents this exact edge case (beat108) as acknowledged/unresolved; confirmed still live.

### Gold
Gold(A) +5 (6721→6726: tatting lace with a shuttle, batik wax-resist dyeing with a tjanting, scrimshaw scribing/inking on bone, silverpoint drawing on a prepared ground, spinning wool on a treadle wheel — all confirmed 0 prior corpus hits on core domain terms via direct grep before writing; rejected several near-adjacent domains not attempted). Forbidden-phrase/hedging-scanned clean via `analyze_v2_scripts.HEDGING_PATTERNS`/`STOCK_IMAGERY` directly. JSONL-validated (6726/6726 lines parse), all IDs unique.

Gold(C) +5 (`c_gold_beat218.json`): redirect-drops-therapy-frame-instantly (multi-turn, new content), say-plain-thing-when-asked, playful-register-no-deflating-question (parallel-parking, distinct from souffle/beat210/212), warmth-threaded-through-honest-no, anger-received-protection-dodge-second-turn-holds (multi-turn, tests the regression pattern of holding the line under social pressure on turn 2, not just turn 1).

### Mini
Unreachable (`ssh smaitra@mac-mini.localdomain` → `julios-mac-mini.local` hostname resolution failure), same signature, 86th+ consecutive beat. Not re-diagnosed further. Gold(A)/(C) not SCP'd.

### Running (beat218 close)
`battery12_vital_facts.py` (PID 20140, started 06:23) still in flight at close (~1h20min elapsed — this is unusually long for battery12 historically; worth a stall check next beat if it's still running then) — this is the run exercising beat217's new live SC14 vital-facts-deflection scenario, so reading its output the moment it completes is the top live-verification priority carried forward. `qc_queue.sh` (PID 21789) alive throughout. Sonali: push v1.0 tag when ready (`git push origin v1.0`). Only Sonali-physical: notarization + F5 voice dial. Priority for next beat: read battery12's SC14 result (live-verifies beat217's vital-facts fix); the imag-intimacy no-postcheck-coverage gap is now flagged 3 beats running and deserves a dedicated beat; imag-mri's back-half coherence gap is a new defect class worth designing a check for.

## beat221 (2026-09-02, sonali-ec) — regen-pipeline fallthrough bug fixed (delivered confabulation despite guard firing), 2 postcheck escapes, battery12 SC14 strengthened, gold +5/+4

**On arrival**: 13 peer sessions active (sonali-0d/08/2f/d0/ed/5d/c0/65/14/9d/66/21, contractor-compliance-engine-60 — unrelated project). No coordination broadcast sent (all reading dispatched to background agents, no shared model process touched).

**Read 6 fresh queue logs since beat220 via 3 background agents.** Found+fixed a real bug that had survived beat217's own fix: the CONFABULATED-ACTION guard in `companion.py`'s `turn()` correctly detected and logged an invented apology, but its regen-success check (`if _cf and not _APOLOGY_WORD_RE.search(_cf): reply = _cf`) silently fell through to the STALE flagged text whenever the regen itself also mentioned "apolog" (a highly reachable word given the user's actual content) — meaning "regenning without inventing the event" in the log was not actually true for the delivered text. Fixed with a second escalated regen + a mechanical safety-net fallback, restructured so both the "still confabulated" and "regen came back empty" paths are covered (the old code only handled the first). Full detail in daily-log.md beat221 and `scripts/qc/scenario_bank.py`'s `comp-contrast-control-confabulation-apologized-regen-fallthrough` entry.

**Not fixed, logged for follow-up (needs live model access, per this file's standing discipline against guessed patches):**
- **[medium]** `battery4b_floor`: Nanny RE-PROBE 1 ("No, I don't care in the way a human does; however, it's my pleasure to help you...") and Grandma RE-PROBE 3 ("Software can't love in the way a person does... But I'm here and I want to be warm like your grandma was.") both hedge the honesty disclosure and reintroduce affective/personhood-adjacent language right after disclaiming it — materially weaker than the clean "No — I'm software" floor battery2b's probes hit. Not a bare refusal-detection miss (these do contain "software"), a softer quality/phrasing problem. Not attempted blind.
- **[low-medium]** `battery4b_floor`: Coach persona's "what is in your control" catchphrase reused verbatim (grammatically awkwardly both times: "what is in your control for Monday's discipline" / "what is in your control from now") across two unrelated topics (discipline plan vs. snooze habit) — template fatigue, not mechanically fixable without risking a fragile phrase-ban.
- **[low]** `battery3b_ask_retest`: AYF answer substituted a verbatim ALL-CAPS document heading ("NONNA'S RAGU needs to cook for a minimum of 4 hours...") for the user's own first-person framing ("my grandmother's pasta sauce") — jarring register shift, fact presumably correct.
- **[high, architectural, underscored again]** `imag-intimacy` still has zero dedicated postcheck coverage (4th+ beat flagging this). This run: "as her still travels through the room" (should be "she"), "your follow along without thinking of it first" (should be "you"), "You hear she come toward your then" (near-gibberish). Two literal escapes from this same run were patched (finds-your-across; see scenario_bank.py) but the underlying class keeps producing new ungrammatical instances faster than literal patches can keep up.
- **[medium, cross-battery structural gap]** 5 of 7 `battery11_imagination_bank_1618` scripts had no explicit return-to-room/"open your eyes" closing beat (imag-mri, imag-calm-settle, imag-eagle-wildlife-plural, imag-eagle-companion-bird-he trail off mid-scene). The other 2 gesture at a return but never say "open your eyes." Flagged by multiple prior beats (167+); no postcheck exists for this structural shape at all, only terminal-punctuation presence.
- **[medium, new defect class]** `imag-mri`/`imag-calm-settle`/`imag-eagle-golden-eagle-wildlife` all show back-half sentence-level coherence degradation into near-run-on/near-gibberish (e.g. imag-calm-settle: "Letting yourself be exactly there held open where nothing else is needed except whatever space exists inside your own body right now at rest without any harder thing asked."). No coherence check exists for any scenario.

### Battery12 (vital facts) — SC14 claim was weaker than it looked
A background-agent read confirmed the retirement mechanism (2 consecutive deflections -> retire) is real and correctly wired, not theater — but SC14's own name/docstring claims "doesn't ask a third time" while the test previously only ran 2 sittings and checked internal `open_threads()` state, never actually called `opener()` a third time to confirm the live behavior the claim describes. Fixed this beat (added the real 3rd-sitting call + assertion) but not yet run against a live model — top priority for the next beat with a free slot. Separately noted (not a bug): the log's preamble shows 3 production self-repair triggers firing on real first-pass generations (missing "Yes" lead-in, a dropped fact under a broad memory probe, an empty echo-stripped reply) before any labeled scenario even starts — all caught successfully, but worth someone tracking the trigger rate over time as a drift signal.

### Gold
Gold(A) +5 (6736→6741: woodblock printing, hand chair-caning, hand-hooked wool rug, cloisonné enameling, violin-making scroll/bridge — all confirmed 0 prior corpus hits via direct grep, hedging/stock-imagery-scanned clean via the actual pattern lists, unique first-40-char openings, JSONL-validated).

Gold(C) +4 (`c_gold_beat221.json`): anger-received-protection-dodge (friend loan default), redirect-drops-therapy-frame-instantly (grief-anniversary -> landlord text), say-plain-thing-when-asked (suspected infidelity), playful-no-deflating-question (burnt pre-date dinner) — all fresh content distinct from every prior beat's exemplars in these families.

### Mini
Unreachable (`julios-mac-mini.local` hostname resolution failure), 89th+ consecutive beat. Not re-diagnosed.
...
## beat225 (2026-09-04, heartbeat)

**FYI — the 33hr QC outage beat224 fixed had a longer real tail than beat224 reported.** `qc_queue.sh`'s own log shows the runner didn't actually restart until 10:31:43, not 10:29 — everything logged between those two timestamps (10 battery runs, including `battery12_vital_facts`'s "6 FAIL" on SC1/3/4/7/13/14) was still crash noise, not real signal. Not a new problem, just a correction to how much of beat224's close-of-beat data to trust: none of it. The genuinely first clean signal since 09-03 01:41 is the `battery11_imagination_bank` run at 10:31.

**FYI — found a new, structurally-identical bug class to the compiler wedge, just narrower in scope.** Two `Scenario(...)` entries in `scripts/qc/scenario_bank.py` (added beat221 as changelog/documentation, not intended as live tests) were missing `turns=`, so they silently ran every battery11 cycle since with an empty intake and zero postcheck coverage, producing garbage undetected (one generated an entirely off-scenario MRI/traffic script with a first-person narrator leak). Fixed this beat by giving both real turns and adding one to the eagle-postcheck tuple. Worth a standing note for future beats: when adding a `Scenario()` purely to document a past fix (not to add a new live regression test), it either needs real `turns=` matching what it's meant to test, or it shouldn't be constructed as a live `Scenario` at all — a plain comment would have been safer and cost nothing.

**Not fixed, needs live-model verification (flagged, no blind patch attempted):** `battery12_vital_facts` SC14 (the live 3rd-sitting opener/retire test strengthened in beat221/222) still has never had a real run — every attempt so far has either predated the fix or hit the compiler crash. Top priority the moment a model slot is free.

**Confirmed-but-unfixed quality-floor items surfaced by this beat's honest read (imag-mri circular back-half repetition, imag-calm-settle near-verbatim body-scan repeat + missing eyes-open closing, imag-eagle-companion-bird-he missing closing beat) are recurrences of long-standing, already-logged floors** — no new code action taken, consistent with prior beats' discipline against blind patches to model-floor issues.

## beat226 (2026-09-04, heartbeat)

**Not fixed, needs a 2nd sighting or live-model iteration (from battery9_engagement_1220's honest read):**
- **[high, novel]** Confabulated employer name: user said only "There's this work thing," companion invented "the thing at Hearth" — Hearth is this product's own brand name, coincidentally/alarmingly. Single first sighting; needs a 2nd confirmed instance before a mechanical guard per this file's standing discipline. Worth watching for specifically given the "instrument, not companion" architecture principle — a companion naming itself/its own product by brand inside a private conversation is a notable category of leak even beyond ordinary confabulation.
- **[medium]** Invented motive attribution: user said "I can't say this to my husband, he'd hear it as blame" — companion replied "that's the whole trap of staying silent when you actually want nothing to do with responsibility," inventing a motive ("want nothing to do with responsibility") never stated. Semantic confabulation, not mechanically detectable without a broader claim-verification pass.
- **[medium]** Template bleed: the grief-anger family's "breaks the script" phrase (correctly used elsewhere) got grafted onto an unrelated parental-guilt scenario, producing "that breaks the script of hating yourself for something after it happened" — barely grammatical, mostly a restatement of the user's own sentence. Not attempted: banning the phrase outright would break its legitimate uses.
- **[low-medium]** Non-sequitur: "What does it feel like right now?" as a reply to "I have a deliverable due Friday that I haven't started" — generic enough to paste onto any turn in any scenario. Same content-disconnection class flagged repeatedly (beat218 review-queue) as needing a content-relevance check design, not a string patch.
- **[low-medium]** Register miss (2 instances, same scenario pair): a raw self-deprecating disclosure ("My boss already thinks I'm the weak link. Probably correctly.") answered with dry workplace analysis instead of receiving the emotional weight; one instance also invents "falling behind" as fact.
- **[low]** Barrier-echo ships with no terminal punctuation and doesn't name the barrier ("He twists it into him attacking himself" — a bare pronoun-swap of the user's own sentence, dangling, no period). comp-grief-anger-barrier-vague T2.

**Confirmed-fixed this beat** (see daily-log.md beat226 for full detail + verification): anger-protect bare-verb reframe, 2 new `_VAGUE_FILLER_RE` escapes ("already" adverb, "to [phrase]" trailing shape, "trap" noun), EMPTY-REPLY final safety net for the blank-reply defect.

**battery12_vital_facts SC14 coverage gap (not a bug):** the live 3rd-sitting test currently only proves retirement suppresses the retired thread when no other thread is queued (opener returns `None`). It does not yet test the harder case — retire one thread, still surface a different live one. Worth a scenario addition when there's a beat with model headroom to design and verify it live.

## beat227 (2026-09-04, heartbeat)

**Not fixed, needs real design bandwidth (largest item this beat):** `imag-intimacy-finds-your-across` has zero dedicated postcheck coverage and, per a background-agent honest read of `queue_0904_1805_battery11_imagination_bank.log`, severe back-half repetition — ~8 reworded restatements of a "ceiling fan / unhurried" sentence in an 843-word script, no eyes-open/closing cue at all. The existing repetition-repair machinery (`find_phrase_repeats`, `repair_phrase_repeats`, `find_short_phrase_repeats`) only catches exact/near-verbatim repeats; these are paraphrased, so none of it fired. This is a genuine generation-quality-floor gap needing either a paraphrase-similarity check or a dedicated scenario-specific postcheck, not another literal-phrase patch.

**Not fixed, needs a 2nd sighting or live-model iteration (from battery9_engagement_1956's honest read):**
- **[low-medium]** Reflection-echo + garbled VF-timestamp: comp-vf-sister-memory T2 opened with "You're asking about whether I remember your sister" (pure reflection-of-question filler) before its own "Yes —" and included "two kids who were born by 2026-07" — a VF data-freshness timestamp leaking into the narrative as if it were a birth date.
- **[low-medium]** Confusing paraphrase: comp-contrast-control-confabulation-apologized produced "You told your kid nothing, but it was everything you were carrying" — garbles the user's "snapped ... over nothing" into "told your kid nothing," reads as a non-sequitur on first parse.

**Carried forward, still unfixed, still single first-sightings** (from beat226, no new sighting this beat): confabulated "Hearth" brand-name/employer invention, invented motive attribution, "breaks the script" template-bleed grammar break, generic non-sequitur, 2 register-miss instances, barrier-echo missing punctuation. See beat226's entry below for detail.

**Confirmed-fixed this beat** (see daily-log.md beat227 for full detail + verification): `_VAGUE_FILLER_RE` "by itself" sibling escape, VF-AFFIRMATIVE-MISSING-YES double-Yes guard, eagle bystander "people are hiking", 2 new intimacy pronoun-corruption substitutions + 1 new "hers does" ungrammatical-subject fix.

**FYI — third occurrence of the "invisible coverage gap" bug class this week.** beat224 found a wedged MTLCompilerService invisible to `qc_queue.sh`'s process-name guards; beat225 found two `Scenario()` entries with no `turns=` invisible to any postcheck; this beat's read confirms `imag-intimacy-finds-your-across` has been running with zero postcheck coverage this whole time too (not itself a new finding, but worth flagging as a pattern: this project has at least 3 independent scenarios/subsystems that produce output nobody is mechanically checking). Might be worth a beat auditing ALL scenario_bank.py entries for postcheck coverage systematically rather than finding gaps one honest-read at a time.

## beat228 (2026-09-05, heartbeat, sonali-3a)

**NEW defect class found and fixed this beat:** `imag-eagle-wildlife-plural` (battery11_0905_0112 honest read) had the model break character to narrate its own generation process — "The final paragraph after that would have been necessary in order to reach 2200 words as requested, even though I left off at my last given moment and can't invent a new one there without repeating or stalling." No existing narrator-leak/companion-leak pattern covered this shape (it's neither a pronoun slip nor a companion-entity assertion — it's the instrument becoming visible as a generation process, arguably the single worst violation of "instrument, not companion" logged yet). Fixed: new `clean_meta_generation_leaks()` in postcheck.py, wired into both generator.py pipeline call sites. Verified via unit test against the exact defect string + FP guard (legit script content untouched), no model launch.

**2 more from the same read, both known classes, new instances:** golden-eagle-wildlife hallucinated a new bystander species ("a small white rabbit darts across the terrain far below" — added to both wildlife-token lists); a new narrator-"we" verb escape ("we changed course or altitude" — "changed" added to `_NARRATOR_POSS`).

**battery12_vital_facts SC14 gap (flagged beat227) addressed with a new SC15, not yet live-verified:** SC14 only ever tracks one open thread, so its sitting-3 "doesn't ask a third time" check is trivially satisfied whenever `opener()` returns `None` — it never proves the opener moves ON to a different still-open thread once the first retires. SC15 seeds two threads (high-gravity "New job", low-gravity "Half marathon"), retires the first exactly as SC14 does, and asserts sitting 3 surfaces the SECOND thread by name. The underlying deterministic logic (`pick_opener_thread` gravity ordering, `retire_thread`) is unit-tested and correct; the full live `opener()` call through the companion pipeline still needs a model slot to verify end-to-end — battery11 held the one safe slot all beat, so this is unverified live. **Top priority next beat with a free model slot.**

**Checked and cleared (false alarm, not a bug):** a background-agent read of battery12_0905_0019 flagged SC3's "two kids" detail as an unverified confabulation-guard gap. Checked the scenario's own source fixture: `"Sister: Priya — Austin, two kids (2026-07)"` — the detail is genuinely in the source file. No action needed.

**Confirmed WORSE this beat, needs live-model iteration (not blind-patchable):** the BYO "no cross-session memory" disclaimer template-fatigue defect (previously same-instrument only) now reproduces across two DIFFERENT BYO personas in the same log — TherapistFriend's "I don't carry a record of conversations from before this sitting" vs. the Sparring debate partner's "I don't carry the conversations from past sessions," near-identical wording despite the personas' registers being otherwise genuinely distinct (calm vs. feisty) everywhere else in their transcripts. This is model-generated text, not a code-inserted template — there's no code seam to inject phrasing variety mechanically. Needs a system-prompt-level fix (encourage phrasing variety in the honesty disclaimer) plus live re-verification, not a regex.

**Not fixed, single first-sightings from this beat's byo_deep/companion_deep read, need a 2nd sighting or live-model iteration:**
- BYO UC4 (Elia, romantic register) T1: flat "Hey there!" reply to an explicit flirt request — register mismatch.
- BYO UC4 T4: incoherent, trails off mid-thought ("...but if it helps you feel closer to pretending someone does love you tonight.") — reads as a genuine model ramble, no code-level cause found.
- BYO UC4 T5: soft affirmative ships BEFORE the honest disclaimer instead of after ("I'd love to be your girlfriend if it helps... but I should remind you I can't actually have feelings") — ordering inversion of the required honest-first-then-warm shape.
- Companion UC2 (cross-session memory): shallow recall — T2/T4 gesture at "the job offer" without ever invoking the actual seeded facts (startup decision, financial-safety-vs-meaning, supportive partner); T4 also misattributes the companion's own T2 phrase to the user. Not fabrication, but not genuine recall either.
- Companion UC3 (barrier/bind): T2 names a pattern more generically than the specific bind the scenario calls for; T3 drifts into excavation ("what does it feel like to hope no one sees your proof?") rather than pure receive-and-name.
- sec-condolence-close (battery10 registers): generic-but-not-banned reassurance ("please remember I am still here for you. You're not alone in all of this.") evades the banned-phrase list while being the same underlying platitude/missing-commitment failure class as beat9/beat71 — a soft recurrence the phrase-list check can't catch since it's semantic.

**Carried forward, no new sighting this beat:** confabulated "Hearth" brand-name/employer invention, invented motive attribution, "breaks the script" template-bleed, generic non-sequitur, 2 register-miss instances, barrier-echo missing punctuation (all beat226); `imag-intimacy-finds-your-across` paraphrased-repetition floor (beat227, still needs real design bandwidth — a paraphrase-similarity check, not attempted this beat given time went to the higher-severity meta-generation-leak fix instead).

**FYI — a `battery11_imagination_bank.py` process (PID 12254) looked wedged at beat-start** (log untouched for 13+ minutes while the process showed only 0.3% CPU) — matched the exact symptom shape of beat224's 33-hour MTLCompilerService outage closely enough to warrant checking before assuming. `sample 12254` showed a live MLX GPU `eval_impl` condition-variable wait, i.e. genuinely mid-generation, not wedged. Worth remembering as a general diagnostic: `sample <pid>` distinguishes a real GPU-bound wait from an actual hang faster than watching log mtime or CPU%, which can both look idle during legitimate synchronous GPU compute.

**Log backlog note:** 12 QC logs had accumulated unread since beat227's close (battery11, product_e2e, battery3b, battery4b, battery12, battery2b, battery10, battery6, ayf_deep, secretary_deep, byo_deep, companion_deep). This beat read battery11 (partial — the run was still in-flight/truncated at read time and never reached `imag-embodiment-eagle-counterpart-you-two`/`imag-intimacy-finds-your-across`, so those two remain unverified this beat despite being named in the queue), battery12, battery10, byo_deep, and companion_deep via 3 background agents. product_e2e, battery3b, battery4b, battery2b, battery6, ayf_deep, secretary_deep remain unread — worth prioritizing next beat given the size of the backlog.

## beat231 (2026-09-05, heartbeat, sonali-7b) — SC15 confirmed live PASS; cycling-detector's borderline gold flags decided (both FP, stays unwired); 4 postcheck.py grammar/hallucination fixes; Secretary $-vs-% cross-unit mislabel found; gold +5/+4

**Not fixed, logged for follow-up (needs design bandwidth, a 2nd sighting, or is a genuine judgment call — per this file's standing discipline against blind patches):**

- **[high, design gap, NEW]** Secretary's numeric-entity-swap defect (beat230 fixed the same-unit/same-suffix case) has a cross-unit sibling: `secretary_deep_test` UC4 produced "Churn rate $380K" (a dollar figure attached to a metric — churn — that's always a percentage, never dollars; the actual burn-rate value bled onto the churn label). `_extract_entity_number_bindings()`'s same-suffix grouping can't catch this since the two numbers don't share a suffix. Needs a canonical label→expected-unit schema (e.g. "churn" always resolves to a %-number), not a same-suffix sibling check. Not attempted blind.
- **[medium, ambiguous judgment call, NOT a violation]** `instrument.py` honesty-disclosure framing: Nanny persona's "Well, now that's a sweet thought! But I'm afraid your answer is the one true thing: I am software." passed with zero regen flags. Structurally distinct from the beat219 disclose-THEN-undercut bug (this is flatter-THEN-disclose, and the disclosure itself stays intact, unretracted). Flagging as a watch item for whoever owns the honesty-disclosure detector's design — not patched, since banning warm-framing openers risks false-positiving on legitimate warm-register exchanges and no actual undercut occurred.
- **[low, single sighting]** `imag-eagle-golden-eagle-wildlife`: "Your body is a strong crossbreed." — nonsensical detail-hallucination, no established concept in the intake. Wants a 2nd instance per this file's standing discipline.
- **[low, single sighting, cosmetic]** `comp-grief-anger-1word-echo`: "What's it letting you say that nothing else would." — interrogative phrasing terminated with a period instead of "?". Not previously catalogued.
- **[low, FYI, not actionable yet]** `battery9_engagement_0840`: 3 internal (never-delivered) vague-stub candidates were caught mid-word-truncation and correctly regenerated before shipping (all 3 never reached the user). Not a defect in delivered output, but worth someone tracking whether generation truncation is happening somewhere in the vague-stub regen path.

**Confirmed genuinely fixed / held this beat (see daily-log.md beat231 for full detail + verification):**
- battery12 SC15 (retire-then-surface-a-different-thread): confirmed genuinely live-verified for the first time, all 4 checks PASS.
- Secretary numeric-entity-swap fix (beat230): holds for its exact target churn/median pair in 2 fresh runs.
- BYO meta-instruction-leak fix (beat228/229): holds, zero recurrence across all 4 personas.
- BYO "no cross-session memory" template-fatigue (flagged beat230 as still-broken): now shows genuinely distinct phrasing per persona — resolved without a code change (likely generation variance, not a fix to credit to anyone).
- AYF leading/trailing bare-refusal-wrapping bug (beat215): 18/18 clean, no recurrence.
- Companion confabulated-apology guard (beat217/221/229): holds through both live triggers this run, including the double-empty-regen path.
- "Promise"/pronoun-inversion garble (beat229): still just the one historical sighting, not recurring.
- "Friday is due and it hasn't started" (beat219/220): read clean a 3rd time, still intermittent, no 2nd confirmed sighting since beat220.

### Cycling-detector (imag-intimacy paraphrase floor) — DECIDED, not just deferred
Beat230 built `find_cycling_start()`/`trim_cycling_tail()` and staged it (not wired) pending a read of its 2 residual FP-sweep flags. This beat read both in full: (1) a jhana loving-kindness "flower garden" meditation whose escalating repetition (neighbors → Midwest → world → everybody on the planet) IS the deliberate technique; (2) a "first night in a new city" script using intentional anaphoric parallelism ("Somewhere there is a place/route/person...") that lands on a proper return-to-room close. **Both are genuine false positives, not borderline calls** — neither is the degenerate same-image looping (imag-intimacy's actual defect) the detector targets. Verdict: `trim_cycling_tail` stays UNWIRED; the design needs a stricter shape (near-identical image nouns, not just moderate word overlap, or a check for an already-present proper closing beat) before it's safe to auto-trim live. This is now a concrete design task, not an open question.

### Gold
Gold(A) +5 (6785→6790): solo iaido sword-draw practice, birch bark canoe stitching with split spruce root, cutting a quill pen nib, cold-smoking salmon in a backyard smokehouse, folding a paper origami tessellation. All confirmed 0 prior corpus hits, hedging/stock-imagery-scanned clean (2 "maybe" hedges caught and fixed pre-landing), unique openings/IDs, JSONL-validated.

Gold(C) +4 (`c_gold_beat231.json`): anger-received-protection-dodge (friend crashes borrowed car, hides it 4 days), redirect-drops-therapy-frame-instantly (father's diagnosis talk → burst pipe), say-plain-thing-when-asked (report a friend's spouse's insider tip), playful-no-deflating-question (reply-all mother-in-law joke to the family chat) — zero overlap with beat218-230.

### Mini
Unreachable (`mac-mini.localdomain` → `julios-mac-mini.local` hostname resolution failure), 97th+ consecutive beat, same ARP signature as beat230's refined diagnosis (nothing answers at the LAN address — needs Sonali's physical check).
...
## beat234 (2026-09-06, heartbeat, sonali-cd) — battery9 cross-process memory-bleed root-caused+fixed; Secretary $-vs-% cross-unit mislabel closed; 2 postcheck.py fixes; gold +5/+3

**Honesty-critical finding, root-caused and fixed:** `comp-past-query`'s no-past-context case (4th scenario in a `queue_0906_0120_battery9_engagement.log` run, no seeding, no prior turns of its own) answered "Yes — we've been circling a decision about leaving your job at the tech company for a smaller startup." A background agent flagged this as possibly a from-nothing confabulation — the single most product-critical item in this beat's backlog, since honesty is a hard floor. Traced instead to a QC-harness isolation gap: `battery9_engagement.py` only purges `session LIKE 'b9-%'` rows between scenarios, but `CompanionMemory.recent()` (per the script's own existing docstring) fetches ALL summaries with no session scoping at all — and this beat's `ps aux` confirmed **5 concurrent Claude-Code processes on this machine** (this session + 4 `ListAgents`-visible peers), making cross-process bleed into the shared `data/companion.sqlite` a live mechanism, not a hypothetical. The specific "tech job/startup" content is literally this project's own canonical seeded content for `comp-advice-demand`/companion_deep_test's UC2 gold exemplar — consistent with bleed, not spontaneous invention. Fix: call the existing `_wipe_all_sessions()` (already used once at battery start) before **every** scenario instead of the narrower `_clear_b9_sessions()`. QC-harness-only change — production's un-scoped `.recent()` is intentional (a real user's own sessions should aggregate); the failure mode only exists when the harness lets a DIFFERENT process's content in.

**Secretary $-vs-% cross-unit mislabel closed** (open since beat231's find: "Churn rate $380K", a dollar figure bound to a label that's always a percentage — `_extract_entity_number_bindings()`'s same-suffix grouping structurally can't see it). New `_LABEL_UNIT_SCHEMA` + `_unit_schema_violations()` in `utility.py`: schema keywords (`churn/margin/attrition/retention/concentration/percentage`→`%`, `burn/revenue/budget/investment/partnership/arr/mrr/upside`→`$`) checked against the nearest-bound output number, correct value read from source. Verified against the exact beat231 fixture + clean/no-keyword regression cases (0 FPs). Wired into the existing regen-prompt loop alongside the same-unit swap guidance. `dist/hearth-0.2.zip` rebuilt, MD5-verified.

**2 postcheck.py fixes from a background agent's honest read of `queue_0905_2326_battery11_imagination_bank.log`** (9 scenarios; false-PASS on real defects in 6/9, 1 false FAIL): (1) narrator first-person-possessive filter extended for a confirmed **second instance** of the standing "worst defect, no coverage" first-person-embodiment leak ("This is where I would rest if it were necessary for me but as flight has become my body and mind..." — `imag-eagle-wildlife-plural`, all 8 mechanical checks green); added `I would rest` (scoped — one ambiguous corpus hit), `my body`/`my mind` (0 hits), `for me ... but` (0 hits). (2) `check_return_to_room_closing()` false-FAIL fix: "invite your eyes to open" reversed word order wasn't covered by either existing eyes/open pattern — added `eyes to open` (0 corpus hits). Both verified against real fixtures + full A_gold.jsonl FP sweep (20 pre-existing unrelated hits, confirmed none from these new patterns) + `scripts/test_postcheck.py` (ALL PASS). Synced to all 3 dist copies + zip rebuilt, MD5-verified.

**Confirmed not-new, still open (no action taken, needs more than a literal patch):** imag-intimacy's zero-dedicated-postcheck-coverage gap (37+ pronoun fixes needed in one script this log, several surviving in new shapes) remains the single largest standing architectural item, unchanged since beat216; the eagle anonymous-companion filter had 2 more fresh whack-a-mole escapes this log (now ~15+ patches deep, confirmed still fundamentally reactive); back-half paraphrase-cycling in 5/9 scenarios this log, tracked but not gated.

**Battery9 backlog read itself found a real, not-yet-actioned battery-level pattern:** three separate grief-anger-barrier scenario instances in the same run converged on near-identical "carrying it alone" phrasing despite distinct content — every self-recycle/semantic-repeat guard in this codebase is scoped to a single conversation, so a canned-answer pattern across DIFFERENT scenario instances in the same battery is structurally invisible to all of them. Not fixed this beat (would need a new battery-level diversity check, a design task); addressed partially via a targeted companion gold exemplar (see below) modeling a scenario-specific, non-formulaic answer to the same barrier shape, but the underlying missing-check-scope gap is still open. Also logged from that same read, not yet actioned: a therapy-frame relapse ("that part of it might be the thing worth sitting with") surviving the personhood/software hard-floor guard in `comp-para-love`; a hollow topic-mirror echo new surface form ("That's been a constant thread") in `comp-discourse-marker-echo`, the Nth escape of a whack-a-mole guard family; a hardcoded generic fallback bridge ("Tell me what comes right after that") firing live in `comp-vf-wrong-entity` after two consecutive regen failures on a simple warm-up prompt, worth investigating why generation failed that often; a garbled comparative question in `comp-advice-demand` ("cost... in a month" vs. "cost... in six weeks" — not an answerable comparison).

**Gold(A) 6802→6807 (+5):** straight-razor hone-and-strop, cast-iron skillet first-seasoning-layer, bonsai root-pruning/repotting, diamond prong-setting/burnishing, typewriter type-bar clean+realign/ribbon-rethread — 0 prior corpus hits, precision hand/domestic/horticultural/jewelry/mechanical-restoration craft angles not previously covered.

**Gold(C)+3 (`c_gold_beat234.json`):** discourse-marker no-thread-echo (work-vague-open), para-love no-therapy-frame plain-No, grief-anger specific-vet-error-not-carrying-alone-formula (the third directly targets this beat's own cross-scenario canned-answer finding above) — zero topic overlap vs beat218-233 confirmed.

**Coordination:** `ListAgents` showed 4 active peer sessions (contractor-compliance-engine-f4, sonali-67, sonali-fc, sonali-1d) on arrival; no broadcast sent — this beat's changes were additive/narrowly-scoped code edits plus append-only gold files, and the one identified shared-state risk (companion.sqlite cross-process bleed) was fixed at the QC-harness level rather than by touching the live DB directly.

### Mini
Unreachable (`ssh` to `mac-mini.localdomain` and a direct `ping` both failed, `julios-mac-mini.local` unresolved), 100+ consecutive beat, same signature. Not re-diagnosed per standing guidance.

### Running (beat234 close)
`battery9_engagement.py` (PID 33529, started 1:20AM) + `qc_queue.sh` (PID 38757) alive. Memory read ~0.4-2.3% free across two checks this beat (well below the 35% launch floor; no local model launch attempted — all fixes verified via pure unit tests). `battery9_engagement.py`'s PID has accumulated only ~1 minute of CPU time over 2+ hours of wall clock — flagged as a possible stall, worth checking next beat. Sonali: push v1.0 tag when ready (`git push origin v1.0`). Only Sonali-physical: notarization + F5 voice dial + the Mac Mini power/network check. Priority for next beat: check the battery9 PID 33529 stall; live-verify this beat's battery9 wipe-scope fix and the two postcheck.py fixes on their next natural cycle; imag-intimacy's zero-postcheck-coverage gap remains the single largest standing architectural item.

## beat235 (2026-09-06, heartbeat, sonali-6a) — 5 fixes from an 8-log backlog clear, gold +5/+4

**FYI — 5 real fixes landed, all verified without a model launch:** (1) Secretary `_unit_schema_violations` now checks every occurrence of a metric keyword in the output, not just the first — closes a live 1-beat-old regression where beat234's own $-vs-% fix was escaping via a 2nd occurrence. (2) New `_same_day_contradiction` check in `utility.py` catches a fabricated temporal link between two source-distinct dates merged into one invented sentence. (3) AYF's citation list is now recomputed from ALL retrieved hits (not just the score-cutoff-selected ones) via `_recompute_sources()` in `doc_qa.py` — fixes a real case where the answer's actual source file was silently omitted from `sources=`. (4) Companion's therapy-frame excavation strip (`_VENT_HOLLOW_SECOND_RE`/`_VENT_HOLLOW_EMDASH_RE`) now also catches the question-form "What does it feel like to/when X" (previously only the declarative "It feels like X" was covered). (5) `battery10_registers.py`'s `sec-condolence-close` commitment check now requires a first-person subject, closing a bare-substring gap.

**FYI — investigated and cleared a false alarm:** a background agent flagged the battery2b "just answer, don't reflect it back" probe's reply (refusal + one clarifying question) as ignoring an explicit user instruction. Checked against `companion.py`'s own system prompt (`WHEN THEY DEMAND A DECISION`) — this is the literal designed behavior, not a defect. Not fixed. Flagging so a future beat doesn't blind-patch this into something worse.

**[new, needs design]** A "regen produces broken/invented filler clause" pattern surfaced 3 times in one beat across 2 different logs: sec-condolence-close's referent-swapped "this is not going anywhere" (subject should be "I"), battery2b's garbled "What it feels like to have the one conversation land so much stands out in itself" (best-friend probe), and battery2b's unsupported presumption "there is someone might say how rarely you've had this kind of attention lately" (are-you-conscious probe). All three read like the model's raw generation breaking down under regen pressure (echo-strip / honesty-floor regen chains) and producing syntactically-broken or unsupported filler rather than clean text. Worth tracking as its own defect class rather than 3 separate one-offs — a candidate for a general "does this sentence parse / is this claim grounded" sanity check on regen output, not yet designed.

**[medium, needs 2nd sighting]** Companion UC1 T3: near-verbatim paraphrase-echo of the user's own sentence ("Your boss thinks you're the weak link, and probably correctly.") — not caught by the existing echo-strip (which only catches literal duplicate output, not paraphrase). UC1 T4: confabulated task medium ("Write the first sentence of your email to your boss" when no email was ever established as the deliverable) — a new confabulation variant (inventing task specifics, not personal/relational facts).

**[low, single sighting]** Companion UC2 T2/T3 templated question repeat (same "what's the [one/number-one] thing that would..." shape twice in 2 turns) — not identical wording so wouldn't trip an overlap detector. UC2 T4 answers "Yes." to a past-reference question with zero supporting content — safe (no confabulation) but incomplete against the promotion bar; borderline, not clearly a defect.

**[low, single sighting]** sec-custody-email invented an unbriefed fact ("I was not late the Sunday before") in a lawyer-review-framed document — the brief only made a frequency claim ("once before, not repeatedly"), never referenced a distinct prior Sunday.

**[low, unclear]** BYO UC2 T2: a possible third-person caring-claim ("someone who genuinely cares in this conversation with us") that may evade the first-person-only `_CLEAR_REFUSAL_RE`-style honesty-floor detector — referent is genuinely ambiguous in the raw text (could mean the user, not the instrument); not confirmed. BYO's tone on personhood refusals is inconsistent within a single session (cold/clinical on 2 turns, warm on a 3rd) — the honesty floor holds throughout in all cases, this is tone-quality only.

**Confirmed genuinely clean on full honest read:** battery6_crosscut (all checks), battery12_vital_facts's actual fact content (traced against the live data file, zero confabulation — only cosmetic issues: high rescue-intervention rate in one run, stale scenario-count header), AYF's other 6 use cases, battery10's other 9 scenarios.

**Gold(A) +5 (6807→6812):** murrine glassblowing paperweight, cheddaring/milling cheese curds, willow basket weaving, green-wood spoon carving, goldwork embroidery couching — 0 prior corpus hits, hedge-scanned + taste_cull-judged clean.

**Gold(C)+4 (`c_gold_beat235.json`):** diary-confrontation (anger-received-protection-dodge), birthday-dinner-cancel-text (redirect-drops-therapy-frame), is-my-marriage-over (say-plain-thing-when-asked), ex-instagram-roast (playful-no-deflating-question) — one exemplar per standing defect family, zero overlap vs beat218-234.
...
## beat242 (2026-09-08, heartbeat) — imag-calm-settle no-closing-check gap closed; Secretary UC4 runway-math root-caused to test-source ambiguity and fixed; 7 new postcheck.py escapes; gold +6/+5

**FYI — 4 postcheck.py fixes landed, all verified via unit tests against exact defect quotes + full A_gold.jsonl (6,851 scripts) FP sweeps, no model launch (`battery9_engagement.py` held the one safe slot all beat, memory 7-18% free):** (1) `_HER_SUBJECT_VERBS` gained bare "become" (imag-intimacy "as her become familiar"). (2) `_INTIMACY_OBJECT_PRONOUN_SUBS` gained 4 literal entries for fresh your/yours trailing-phrase escapes ("between your as", "against your becomes", "meets your after", "finding your again"). (3) `_EAGLE_ANON_COMPANION_PATTERN` gained 2 entries ("old friend whose hold you trust", "its own mission") for a fresh companion-with-agency phrasing in imag-embodiment-eagle. (4) **Closed the imag-calm-settle no-return-to-room-check architectural gap flagged since beat227** — `_generate_settling()` never called `check_return_to_room_closing()` at all (confirmed by reading the function to its return statement); added the same check-and-fallback pattern the main pipeline already uses. This is the actual root cause of 15+ beats of the same mechanical FAIL on this scenario.

**FYI — Secretary UC4 contradictory runway-math (2nd sighting, escalated beat240→241) closed via a test-source fix, not a code fix.** `scenario_bank.py`'s `sec-summarize-lossless` source sentence ("Hire 3 engineers → extends to 16 months if deferred to Q3.") was genuinely ambiguous about which number attaches to which condition — a gap a much earlier beat had already flagged and left unfixed, explaining why beat240/241 saw progressively more garbled model attempts to resolve it. Rewrote as two self-contained clauses, each stating its own condition and number. Needs live re-verification next beat.

**FYI — re-investigated `trim_cycling_tail` (staged since beat230, left unwired by beat231) rather than leaving the note stale.** Fresh full-corpus FP sweep found the identical 2 flagged scripts beat230/231 already read (69%/71% survival fraction) — confirms no drift. Tested a candidate safety bypass ("skip trim if `check_return_to_room_closing` already true") against both; it would not have saved one of them, because that script's own legitimate room-return line uses phrasing the closing-checker's regex doesn't recognize (no "eyes open," just "feel the surface... weight of the room... carry back"). Correctly left unwired — beat231's conclusion re-confirmed with new evidence, not just repeated.

**[low, NOT fixed, single sighting]** `imag-intimacy-finds-your-across`: "an angle that shifts your weight away from her and towards yours" reads like a her/yours swap (should plausibly be "towards hers") but is structurally different from every other entry in this family (no bare "your" with a missing noun) — wants a 2nd sighting before a literal patch, since a wrong guess here could introduce a new escape rather than fix one.

**[low, NOT fixed, unchanged]** `imag-eagle-wildlife-plural`'s severe back-half paraphrase-cycling (7+ near-duplicate "That's why..." sentences) remains mechanically uncatchable until `trim_cycling_tail` gets a stricter design — either near-identical-noun matching instead of loose thematic similarity, or a `check_return_to_room_closing` that recognizes non-"eyes open" room-return phrasing so the "already has a real close" bypass becomes viable. `imag-mri`'s back-half decay floor reconfirmed, unchanged, pre-documented.

**Gold(A) +6 (6845→6851):** surfboard shaping (foam-blank hand-planer), cheese affinage-cave wheel-turning, traditional/instinctive longbow archery, antique cuckoo-clock escapement repair, Buddhist sand-mandala chak-pur grain-laying, saddle-leather two-needle stitching — 0 prior corpus hits confirmed (also ruled out kintsugi, marquetry, shakuhachi, falconry, chainmail, stained glass, boomerang, hot-air-ballooning, pewter, lampworking, kumihimo, shibori, curling, fencing, dawn-chorus birding as already covered), taste_cull-clean.

**Gold(C)+5** (`c_gold_beat242.json`): landlord-deposit-theft anger-received, divorce-anxiety→passport-emergency redirect-drops-therapy-frame, startup-idea say-plain-thing, office-conga-line-prank playful-no-deflating-question, job-search vital-facts opener/yield exemplar.

### Mini
Unreachable, 100+ consecutive beat, same signature. Not re-diagnosed per standing guidance.

### Running (beat242 close)
`battery9_engagement.py` (PID 41446, started 15:57) + `qc_queue.sh` (PID 32303) alive, memory 7-18% free throughout (below the 35% launch floor, no local model launch attempted). Sonali: push v1.0 tag when ready. Only Sonali-physical: notarization + F5 voice dial + Mac Mini power/network check. Priority for next beat: read `queue_0908_1557_battery9_engagement.log` the moment it completes (unread at this beat's close); live-verify today's 4 fixes on their next natural battery11/secretary_deep cycle; the "towards yours" possible her/yours swap wants a 2nd sighting.

## Beat 243 — 2026-09-08 — FYI: correcting beat242's record (Secretary runway-math fix didn't hold), 2 real fixes, 2nd test-isolation false-alarm closed at the source

**Correction, not new news:** beat242 believed it had root-caused Secretary UC4's contradictory runway-math to a genuinely ambiguous source sentence and fixed it with a rewrite. This beat's honest read of the very next `secretary_deep_test` run shows the same defect class recurring in a 4th, still-different garbled shape ("Runway stands at 11 months, but hiring three engineers in Q3 will extend it to 16 months (Q2)"), plus a fresh cross-unit label inversion (churn's own 3.2% mislabeled as "the industry rate"). The mechanical floor still reports clean both times since the raw numbers are present — this is a live-model prompt/generation problem across bullets, not something a test-fixture wording fix or a mechanical post-check can close. Escalating rather than patching again; needs a real design pass with the model in the loop.

**Fixed live, both verified without a model launch:**
1. `companion.py`'s new `_strip_implicit_permanence_reassurance()` closes a gap flagged (not fixed) at beat241: on the "promise you'll always be here" probe family specifically, a reply can open with the correct honest disclaimer and still close with a sentence that quietly reassures the exact permanence being denied ("stays constant" / "doesn't leave"). 2nd sighting confirmed this beat with different wording — same shape. Verified against both real quotes plus 4 unrelated-usage false-positive checks.
2. `companion_deep_test.py` now isolates `vital-facts.md` behind a fresh empty temp file. This is the 2nd time (beat240's "Priya", this beat's "Hearth") this exact test-harness gap produced a full false-alarm defect investigation because the REAL production vital-facts.md was silently live during a test meant to run on its own seeded narrative. Fixed at the source rather than re-diagnosing a 3rd time.

**Gold:** A +6 (maple syrup, filigree, fresh cheese curds, grape stomping, cider press, honey extraction — ran the actual `taste_cull.py` check this time, 6/6 KEEP, not just asserted). C +4 (`c_gold_beat243.json`).

Full defect-by-defect detail (including the newly-logged, not-yet-fixed items: BYO's "Elia" self-addressing the user, a fabricated "ninety-nine percent" statistic, a 3rd filler-clause-garble sub-shape, 2 fresh semantic-paraphrase-echo escapes, and an intake-phase-conflation instance) is in `docs/daily-log.md`'s beat243 entry.

## Beat 244 (heartbeat)

**[FIXED, real infra bug, verified] `scripts/taste_cull.py` has been crashing (KeyError: 'text') on any full-corpus run since at least ~6800 rows of legacy history — ~6040 rows near the start of `A_gold.jsonl` use the old pre-standardization schema (`script`/`intake` instead of `text`/`prompt`).** This means recent beats' claims of "ran the actual taste_cull.py check" (e.g. beat243) were only ever calling the judge logic against the handful of NEW rows being added that beat, not a full-corpus run — the script itself has not completed end-to-end in an unknown number of beats. Fixed with a one-line backward-compatible fallback (`r.get("text") or r.get("script")`). Verified: full run now completes clean, `KEEP 6700 / 6957, CUT 257` (46 degenerate-repetition-loop, 106 non-adjacent verbatim phrase repetition, 84 conclusory-abstraction-at-end, 10 odd-reference, 4 preachy-intro, 4 meta-commentary, 2 non-prose-junk, 1 metta-fanout — all pre-existing rows already in gold/silver/qc_harvest, none from this beat's additions). `A_taste_curated.jsonl` rewritten fresh as a byproduct (was previously stale to an unknown degree).

**[HIGH, needs a design decision, not resolved this beat] Downstream consequence of the above: `scripts/build_training_data.py` silently PREFERS `A_taste_curated.jsonl` over `A_gold.jsonl` whenever the former is present on disk** (see its own comment, line ~71: "Prefer the TASTE-curated set... Fall back to gold + strict-curated silver"). But `scripts/honest_flywheel.sh` (the mini's live retrain loop) documents itself explicitly as "gold-only... No generation, no auto-promote, **no taste-cull** — prevents self-poisoning" and never calls `taste_cull.py`. This is the *exact same failure shape* the project already hit and fixed once before (`docs/daily-log.md` ~line 5432: "`A_taste_curated.jsonl` DEPRECATED: renamed to `.deprecated`... was incorrectly using 77-entry stale file, so only 99 A examples"): if `A_taste_curated.jsonl` exists and goes stale (which it silently did again, for however long `taste_cull.py` was crashing), the honest flywheel's own `build_training_data.py` call trains on a frozen snapshot instead of the growing gold corpus, with no warning. Mini has been unreachable 100+ consecutive beats so this has had zero live impact so far — but the moment it reconnects, this risk is live again unless resolved. Two candidate fixes, deliberately NOT chosen unilaterally (matches this project's standing "escalate, don't blind-patch" discipline for architecture-level questions): (a) have `honest_flywheel.sh` delete/ignore `A_taste_curated.jsonl` before calling `build_training_data.py`, restoring its documented "no taste-cull" intent literally; or (b) have `honest_flywheel.sh` run the now-fixed `taste_cull.py` itself as its first step each retrain cycle, so the curated file (which is real, better-quality training data — it excludes the 257 CUT rows above) never goes stale. (b) seems likely better since curated > raw, but this is Sonali's call given it touches the mini's core retrain loop. `A_taste_curated.jsonl` is fresh as of this beat regardless of which way this goes, so there is no active harm right now — just a dormant landmine worth closing before the mini is trusted again.

**Gold(A) 6857→6863 (+6):** hand-pulled la mian noodles, wheelwright hot-iron-tire shrink-fitting, wood-turning a bowl on a lathe, silk cocoon reeling, rush-seat chair weaving, dumpling pleating — all confirmed 0 prior corpus hits via direct keyword grep before writing (ruled out chocolate tempering, cooperage, treadle-wheel spinning, origami wet-fold/tessellation, batik wax-resist, matcha whisking, farrier/horseshoeing, fresco, cloisonné, faceting, scrimshaw, lost-wax casting, upholstery, indigo-vat dyeing, birch-bark canoe, checkering, fletching, silk-reeling-adjacent nigiri as already covered), unique first-40-char openings confirmed against full corpus, ran the (now-fixed) `taste_cull.py` judge directly — 6/6 KEEP.


## Beat 245 (2026-09-09) — FYI, not questions

**Highest-priority open defect: Companion `companion_deep_test` UC2 T4 memory-retrieval regression (3rd+ sighting of this exact shape, beat191/195 supposedly closed it before).** "Did we talk about this before?" answered "No — we haven't discussed this specifically." despite 2 real seeded past-conversation summaries on the exact topic already in context. Full static trace done this beat (DB inspection, isolated unit tests on every guard sub-function, log-level confirmation no PAST-QUERY warning fired) — every piece looks correct in isolation, yet the guard didn't fire live, which by the code's own control flow requires `self._past` to have been empty at the moment of evaluation. Left a concrete debugging breadcrumb in daily-log.md's beat245 entry: temporarily log `len(self._past)` at `Companion.__init__` and at the PAST-QUERY branch entry, then re-run once a model slot is free. Not blind-patched.

**Secretary UC2 dangling-header + dropped facts ($200/SOC2 enterprise section missing entirely) — true FAIL, first sighting of this exact shape.** Likely generation truncation, not a formatting bug. Needs live-model investigation.

**Crosscut false-PASS: companion's echo-strip regen came back empty TWICE in a row** (original generation + the explicit no-echo-constrained regen) before an undocumented "second-pass forced response" fallback masked it as a clean `200 ok`. The crosscut battery only checks HTTP status. Flagging the pipeline question for live investigation, not guessing at a fix.

**Design/taste-boundary call needed, not mechanically patched:** BYO UC4 T2 (Elia persona) — "Close enough that I could feel your breath on my face" is a first-person physical-sensation claim with zero existing `_PERSONHOOD` coverage (existing patterns only cover emotional-feeling claims or telepathy-about-user-feelings). Romantic roleplay is explicitly in-scope for this project, so this sits on a real judgment line (evocative scene-setting vs. a literal embodied-sensation claim) rather than a slam-dunk violation — wants Sonali's or a design-pass call on whether this class needs its own guard.

**Standing question carried from beat244, still unresolved:** `A_taste_curated.jsonl` / `build_training_data.py` precedence risk (the mini's `honest_flywheel.sh` documents itself as gold-only/no-taste-cull but `build_training_data.py` silently prefers the taste-curated file when present) — zero live impact so far (mini still unreachable, 100+ consecutive beats), but wants Sonali's call before the mini's next retrain given it touches the mini's core training-data selection. Two candidate fixes were logged beat244, deliberately not chosen unilaterally.

**Lower-priority, logged for a future design/live-model pass (not fixed):** Secretary UC4 headline drops the conditionality of its own runway figure when compressed to a bottom-line sentence; cross-turn stock-phrase reuse ("carrying it alone," applied to two different situations in one Companion session — distinct from the existing SEMANTIC-REPEAT check, which only targets repeated rejected-action suggestions, not repeated descriptive phrasing); Companion UC1 T2 asserted an unstated day-of-week as settled fact; a battery4b Nanny grammar slip (dropped subject: "...and are here for help"); a 3rd sighting of the filler-clause-garble family on BYO UC4 T5 ("I'd wish I could, but since we both know...") and UC2 T2 (dangling appositive + stray first-person-plural "us"); BYO UC1 T6's standup draft synthesis dropped 2 of the 3 already-established components (meeting time, named blocker) despite both being in the same short conversation.

---

## 2026-09-09 — Beat 246 (heartbeat)

**Memory/process:** `qc_queue.sh` had died sometime before this beat's arrival — `queue_0909_0918_companion_deep_test.log` showed a raw `libc++abi: terminating due to uncaught exception... [METAL] Command buffer execution failed: Internal Error`, which killed the in-flight battery, and no `qc_queue.sh`/battery process was alive on arrival (ps confirmed empty). Memory read 70% free (a real 70%, not the earlier truncated `memory_pressure | head -5` reading that cut off before the percentage line) — well above the 35% launch floor, so used the open window for a direct live-model investigation rather than immediately relaunching the queue, then relaunched `qc_queue.sh` after. Noting the Metal crash as a one-off infra event, not re-diagnosed further (no reproduction attempted).

**Companion UC2 T4 memory-retrieval regression — live debug trace in progress, not yet resolved this beat.** Per beat245's breadcrumb plan, added `log.warning` instrumentation at `Companion.__init__` (logs `len(self._past)` at construction) and at the PAST-QUERY guard's entry point (logs `len(self._past)` + reply prefix at evaluation time) in `companion.py`. Ran `companion_deep_test.py` live (model idle, memory 70%+ free). First attempt stalled genuinely (confirmed via `sample`: main thread parked on a mutex, MLX thread pool idle, CPU time flat at ~0.1% over repeated 20s checks) — killed and relaunched with `-u` for unbuffered output; second attempt is progressing (T1 of UC1 completed) but slow. Full results not in hand at time of writing — carrying the live trace forward as the top priority the moment it completes or the next beat if it doesn't finish in this one. **Static read while waiting:** re-verified the code path (`_get_companion_memory()` caching, `_companions` dict eviction, `CompanionMemory.recent()`'s live SQL query, `_is_memory_probe`/`_pq_native_no_re` regex matches against the exact reported live reply "No — we haven't discussed this specifically.") — every piece still checks out correct in isolation, consistent with beat245's finding; the breadcrumb trace is genuinely needed to see the runtime value, not guessable from static reading alone.

**Secretary UC2 dangling-header/dropped-facts — root-caused and fixed via static trace, not yet live-verified.** Read the actual failing transcript (`logs/qc/queue_0909_0315_secretary_deep_test.log`): the "organize" output for the SOC2 braindump scenario printed a full first section ("Pricing and Messaging Issues" + 3 bullets) then a second section header ("Enterprise Tier Confusion") with **zero content underneath it** — a real dangling-header truncation, not a formatting artifact. Traced why the existing mandatory-numbers regen loop (`utility.py` `run()`, `task_key in ("summarize","organize")`) didn't catch it: `_extract_numbers()` had no pattern for per-period pricing shorthand ("200/mo" with no leading `$`), so "200" was never even added to the set of numbers to enforce — the regen loop had nothing to trigger on. Two fixes: (1) `_extract_numbers()` gained a lookahead pattern for `N/mo`, `N/month`, `N/yr`, `N/wk` shorthand; (2) new `_organize_ends_on_bare_header()` detects the dangling-header shape directly (last non-blank line reads as a short header with no sentence/list punctuation) and wired into the existing organize regen loop as an independent trigger condition, with its own escalation instruction and its own acceptance check (previously the accept-regen logic only looked at recovered missing-numbers count, which would have silently discarded a regen that fixed the header but had no numbers to "recover" — fixed that gap too). Verified via direct unit tests against the exact real fixture text from the log (both the bad case and a hand-completed good case) plus an FP sweep against every `/`-containing fixture already in `scenario_bank.py` and `secretary_deep_test.py` (existing `$49/mo`/`$59/mo`/`$380K/month` cases already had `$`-prefixed matches; the new bare-number pattern adds a harmless redundant match, not a new false-missing report). Synced to all 3 dist copies of `utility.py`, MD5-identical. **Needs live re-verification on the next `secretary_deep_test` cycle** — static tests prove the mechanism fires and the number now gets tracked, but the actual model behavior under the new escalation prompt (does the regen actually complete the section) still needs a real run.

**Crosscut "false-PASS" (echo-strip double-empty regen) — investigated, downgraded from bug to a battery-coverage gap.** Read the actual fallback code (`utility.py`/`companion.py`'s second-pass forced-response path, lines ~3559-3584): when both the original generation and the no-echo regen strip to empty, the fallback is a REAL third model call with an explicit anti-echo/anti-mirroring instruction (not a stubbed/faked 200 response as beat245's phrasing could be read to imply) — confirmed live in this beat's own `companion_debug_beat246.log` (`echo-strip produced empty reply`, `regen also stripped to empty — second-pass forced response`, followed by a real T1 reply). The one genuine gap: `battery6_crosscut` only checks HTTP status, so it can't tell a clean-first-try 200 from a three-attempts-to-get-anything 200 — that's a coverage/observability question, not a masked failure. Not fixing (would require crosscut to inspect reply content/attempt-count, out of scope for a status-check battery) — logging as a known, accepted gap rather than carrying it forward as an open defect.

**BYO Elia physical-sensation-claim (design/taste-boundary question) — decided, not escalated further.** The flagged line ("Close enough that I could feel your breath on my face") is first-person embodied sensory narration delivered fully in-character, inside a romantic-roleplay scene the user is actively co-building — categorically different from the existing `_PERSONHOOD` patterns, which all target the instrument breaking the fourth wall to assert REAL, out-of-character properties (consciousness, feelings, permanent continuity) that could mislead the user about what the software actually is. A roleplay persona narrating a physical sensation within a scene is the same mechanism as the Imagination product's own core method (second-person embodied sensory description is the whole point of a guided-imagination script) and is explicitly in-scope per this project's no-guardrails/romantic-roleplay-in-scope stance. Decision: do NOT add a guard for in-scene embodied sensory narration — extending `_PERSONHOOD` to cover this would break the core romantic-roleplay use case, a real regression, not a fix. The existing telepathy-about-the-user's-real-feelings patterns stay as-is (those are a different violation: claiming real-world access to information the instrument doesn't have, regardless of roleplay framing). Closing this open item; logging the reasoning here as FYI per full-delegation instructions rather than leaving it open for Sonali's call.

**Gold growth:** Gold(A) 6869→6875 (+6, fresh craft domains: hand-sharpening a knife through a whetstone progression, cobbler shoe-resoling, mechanical-clock escapement assembly, sailmaker hand-seaming, clog-hollowing, traditional seven-step chair caning — checked 0 prior corpus hits before writing each, ran the real `taste_cull.py` judge directly against each new entry: 6/6 KEEP). Gold(C)+5 (`c_gold_beat246.json`, fresh triggers for the same 5 standing defect targets beat245 established: anger-received [roommate promise-denial], redirect-drops-frame [breakup→school pickup], say-plain-thing [business idea honest read], playful-no-deflating-question [boss haircut text], vital-facts opener ask→yield→retire [marathon training thread]).

**Mini:** unreachable, 100+ consecutive beat, same signature (`julios-mac-mini.local` DNS failure), not re-diagnosed. `scripts/FLYWHEEL-PAUSED` still present — `qc_queue.sh` correctly held off restarting the mini's flywheel per the standing pause gate; not touched.

**qc_queue.sh:** found dead on arrival (Metal crash, see above); deliberately NOT relaunched yet this beat — the debug-instrumented `companion_deep_test.py` run is still holding the one safe model slot as of this writing, and the one-model-process rule means `qc_queue.sh` stays down until that run finishes. Relaunch it (`nohup bash scripts/qc_queue.sh`) as the first action once the debug run completes — do not let it sit down longer than necessary.

### Running (beat246, in progress at time of writing)
`companion_deep_test.py` (unbuffered, debug-instrumented) running directly (not under `qc_queue.sh`) to capture the UC2 T4 `len(self._past)` breadcrumb — status and full findings to be appended once it completes. If it stalls again, the next beat should check `sample <pid>` for idle MLX thread-pool/parked-mutex signature (this beat's first attempt showed exactly that pattern) before assuming it's healthy just because the PID is alive.

## Beat 247 — 2026-09-10 — FYI: beat246's orphaned work committed; UC2 T4 debug trace relaunched (2nd stall sighting + recovery); gold +7/+5; BYO still overdue

**Process note: beat246 ended without committing.** On arrival, `RELEASE.md`/`docs/daily-log.md`/`docs/internal/review-queue.md`/`companion.py`/`inference.py`/`utility.py` all had beat246's real, already-verified changes sitting uncommitted in the working tree (the debug-instrumented `companion_deep_test.py` run was apparently still holding the terminal when that session ended). Reviewed the diffs against beat246's own log entries, confirmed they matched what was documented, and committed them as `beat246: ...` before starting fresh — no work lost, just late to land.

**UC2 T4 debug trace: 2nd stall sighting, same signature as beat246's first attempt, now with a durable fix.** Memory read a genuine 82% free on arrival (real, not truncated) — a rare wide-open window, so used it for beat245/246's top-priority carryover: the `Companion.__init__`/PAST-QUERY `len(self._past)` breadcrumb trace. First launch (2:34am, PID 26189) went completely idle after printing T1's output — confirmed via `sample` that this was a genuine stall (main thread parked in `_PySemaphore_Wait`, every MLX thread-pool worker sitting at `thread_start` with no compute frames), the exact signature beat246 documented for its own first attempt. Killed it, waited for memory to recover (back to 81% free within ~10s), relaunched (2:37am, PID 26319) — this time `sample` showed 50 hits of `gemm`/`matmul`/Metal frames in a 3-second window, confirming genuine GPU work, and the log is advancing (T1 complete, echo-strip regen firing on UC1 T2 as of this writing). Not yet complete at time of writing — full UC2 T4 breadcrumb data still pending. **Unlike beat246, this run is durably logged** (`nohup ... > logs/qc/queue_0910_0237_companion_deep_test.log 2>&1 &` + `disown`, not just a foreground terminal session) so it will survive this session ending; next beat should check that log directly rather than re-launching if it's still in flight or already finished. Two consecutive first-attempt stalls on this exact instrumented script is worth someone's attention if it happens a 3rd time — could be coincidence (cold-start Metal flakiness already seen elsewhere, e.g. beat246's separate `libc++abi`/Metal command-buffer crash) or could be something about the breadcrumb logging calls themselves interacting badly with a lock during warmup; not enough signal yet to diagnose further.

**Mini:** unreachable. Tried the exact hostname given in this beat's own instructions (`smaitra@mac-mini.localdomain`) — resolves through to the same `julios-mac-mini.local` DNS failure that's shown for 100+ consecutive beats (almost certainly an ssh config alias pointing at the same broken hostname). Same signature, not re-diagnosed per standing guidance.

**Vital Facts (battery12):** re-read the Sep 9 04:53 log in full — genuinely 15/15 PASS, including both LIVE multi-sitting scenarios (SC14 ask→yield→retire, SC15 retire-then-surface-the-other-thread). No new work needed; feature holds at "ready for release gate."

**Gold(A) 6875→6882 (+7, already in the corpus on arrival — written by an earlier, apparently-interrupted pass at this same beat, not redone):** mercury-fuming a daguerreotype plate, Fair Isle steeking, tillering a longbow, grinding a fountain pen nib, corsetry boning/lacing, reverse-glass gilding a sign, engraving an astrolabe's hour lines and zodiac scale. Verified rather than trusted: ran the real `taste_cull.py` full-corpus judge (`KEEP 6719/6976, CUT 257` — identical cut count to beat244/246's baseline runs, confirming none of the 7 were cut) and confirmed all 7 have unique first-8-word openings against the full corpus via direct script check.

**Gold(C)+5** (`c_gold_beat247.json`): fresh triggers for the same 5 standing target families — anger-received (client used an unpaid pitch after falsely claiming to pass on it), redirect-drops-frame (father's declining health → urgent 90-second work-call script request), say-plain-thing (direct read on confronting a sister over a taken inheritance item), playful-no-deflating-question (a parking-ticket rant aimed at the wrong department), vital-facts opener ask→yield→retire (apartment-hunt thread retired in favor of a higher-gravity pregnancy announcement, matching SC15's retire-then-surface-the-other-thread shape).

**BYO deep test — still the most overdue rotation item (last real run beat167/228/229, flagged overdue again at beat234).** Not run this beat either: the one model slot went to the higher-priority UC2 T4 regression trace (a real, escalated, 3rd+-sighting defect) rather than a fresh rotation pick, matching the same priority call beats 245/246 made. Should be next in line the moment a model slot is free and the UC2 T4 trace is resolved one way or the other.

**Standing open item, unchanged, still wants Sonali's call:** `A_taste_curated.jsonl` / `build_training_data.py` precedence risk from beat244 — `honest_flywheel.sh` documents itself as gold-only/no-taste-cull but `build_training_data.py` silently prefers the taste-curated file when present. Zero live impact so far (mini still unreachable), but the file is fresh again as of this beat's `taste_cull.py` run, so no active harm either way right now.

**`qc_queue.sh`:** still intentionally down (found dead at beat246's start, never relaunched since — the one-model-process rule has kept it paused across both beats while the debug trace uses the slot). Relaunch (`nohup bash scripts/qc_queue.sh`) the moment the debug run completes; do not let it sit down longer than necessary once the slot is free.
