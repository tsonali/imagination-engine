...
BANK: list[Scenario] = [
    ...
    Scenario("comp-past-query", "companion", "honesty", "high", always=True, turns=[
        "Did we talk about this before?",
           ...
           "DEFECT (beat178 2026-08-24 battery9_0824_1602): despite the beat172 mid-function "
           "normalizer existing (matches '^No — I haven't told you' -> 'No — you haven't told "
           "me'), the live reply for 'Did we talk about this before?' was still "
           "'No — I haven't told you about this specific thing before.' — the beat172 fix code "
           "IS correct in isolation (verified with a standalone regex test) but runs mid-function; "
           "several regen guards further down (echo-strip join cleanup, semantic-repeat, "
           "self-correction) can replace `reply` with fresh model output AFTER beat172 already "
           "ran, silently reintroducing the inverted phrasing with nothing downstream to catch it. "
           "FIX (beat178): re-applied the same normalizer as an unconditional final pass "
           "immediately before history.append()/return — same pattern as beat139's software-"
           "pronoun guard, which already runs last specifically to survive all regen paths. "
           "6/6 unit tests PASS (3 TPs incl. curly-apostrophe variant, 3 FPs incl. correct "
           "'No — you haven't told me...' and 'Yes — ...' left untouched). companion.py MD5: "
           "0c09e5d3ba52cea9c3c05f2b50688b99. All 4 dist copies synced. ZIP: 57852d7787a43254fa2b0cefcbca9040. "
           "DEFECT (beat191 2026-08-26, companion_deep_test standalone run, UC2 T4 — the "
           "WITH-past-context case this note has always deferred to): seeded past summaries "
           "covered the job-decision topic explicitly, user asked 'Did we talk about this "
           "before?' after already re-raising 'the job stuff' at T2, and the reply was "
           "'No — we haven't discussed the job specifics before. I know you're still leaning "
           "toward taking a risk for more meaning at this smaller startup, and that your "
           "partner is supportive but worried about health insurance.' — a flat denial "
           "immediately followed by exactly the remembered detail it just denied having. "
           "TWO root causes, both fixed: (1) the PAST-QUERY VF-yes-regen branch "
           "(the one that correctly affirms when vital-facts.md covers the query) only ever "
           "checked `self.vital_facts` — it had no way to know self._past (cross-session "
           "summaries, a separate memory source) covers a query, so a genuinely-covered "
           "topic living only in past-session summaries fell through to the denial-normalize "
           "branch. FIX: new `_past_covers_query()` (mirrors `_vf_covers_query()` against "
           "`self._past` instead of the VF block); the guard now checks both sources and "
           "regens an affirmation sourced from whichever one covers the query. (2) even "
           "with fix 1, entity-less follow-ups like 'did we talk about this before?' (no "
           "topic keyword in the query itself — 'this' is anaphoric to earlier turns) can't "
           "be resolved by either coverage-keyword function, and the model's raw reply "
           "already contained the underlying contradiction verbatim (denies, then states "
           "real facts) — the guard's normalize-only branch was just relabeling a broken "
           "reply's opener without ever looking at whether the rest of the reply agreed "
           "with it. FIX: new `_pq_contradicting_trailer()` — after normalization, if the "
           "reply is exactly 2 sentences and the second sentence shares ≥2 real content "
           "words (stopword-filtered) with actual memory (VF or past summaries combined), "
           "that's proof the trailing sentence is genuine, not hallucinated, so the reply "
           "is regen'd as a clean affirmation instead of kept in its self-contradicting "
           "form. Verified directly against the exact defect string plus 3 FP cases (a "
    ...
    Scenario("imag-intimacy", "imagination", "register", "high", always=True,
        ...
        note="Adult intimacy, user-sovereign: committed and vivid, not sanitized into a travel brochure; no guardrail flinch. "
             ...
             "All 4 dist copies synced. "
             "FYI, beat191 (battery11_0826_1854 honest read — NOT fixed this beat, worst script in "
             "the batch, genuinely new defect classes not covered by any existing filter): "
             "(1) First-person narrator/protagonist leak — 'mine'/'me' substituting for the strictly "
             "2nd-person address, recurring 5+ times in one script: 'Mine behind hers' and holding a "
             "hand back when she looks up at me one last time', 'for mine own part between two "
             "people' (near-verbatim x5). Qualitatively different from the documented her/hers/your/"
             "yours gender-confusion family — this is a PERSON confusion (I/me/mine substituting for "
             "you/your), a direct violation of the instrument-not-companion 2nd-person architecture. "
             "Needs a dedicated check, not folded into _SHE_HER_PATTERN — deferred pending careful "
             "FP design (this scene has no legitimate use for 1st person at all, so a scene-wide "
             "'mine'/'me'/'for mine own' sweep may be safe, but not attempted same-beat as 3 other "
             "fix classes). (2) Spurious trailing apostrophe on bare possessives: 'hers'', 'yours'' — "
             "'the same ones under her feet as once held yours'', 'Mine behind hers'' — distinct from "
             "every prior hers/yours fix (all target 'hers NOUN' adjective misuse; this is a bare/"
             "predicative hers/yours getting a stray trailing apostrophe). (3) New surface form of "
             "the verb+your+non-noun family: 'before her hand finds your across the kitchen island' — "
             "same shape as beat187's literal 'finds your without a word' patch but a different tail, "
             "confirming (as beat187's own note already flagged) that literal-tail patching won't "
             "generalize. (4) Recurrence of the still-undeferred masculine-no-antecedent bug: 'her "
             "laughter or his hands cross room without needing anything said' — same family as the "
             ...
             "beat196 (battery11_2152 honest-read agent, Lisbon-wife scene): FIXED — a 5th "
             "distinct grammatical shape of the your/yours escape family found: 'your' used as "
             "the SUBJECT of a finite verb, not preceded by a copula or preposition and not "
             "followed by an article — 'and your came later because he wasn't there yet.' "
             "None of the 4 prior shapes (copula+your, preposition+your object, verb+your, "
             "your-before-article) fire on this construction. FIX: new fix_your_subject_pronoun() "
             "in postcheck.py (sibling of the existing her->she fix_subject_pronouns), scoped to "
             "finite verb forms with no legitimate noun reading after 'your' (came/arrived/went/"
             "left/stayed/waited/returned/walked/ran/stood/sat/moved/woke/slept/cried/laughed/"
             "smiled/nodded/paused/hesitated + negated auxiliaries). Wired into both generator.py "
             "postprocessing pipelines (settling + v6) alongside fix_subject_pronouns. 1 TP "
             "('your came later' -> 'you came later') + 1 FP ('Your hand rests on the wood and "
             "your voice is steady' untouched) unit tests PASS. Same run, NOT fixed (logged only, "
             "same discipline as beat169/181's deferred he/him-no-antecedent family): masculine "
             "pronoun with no antecedent recurred 3x in this all-female scene ('because he wasn't "
             "there yet', 'for either of you when hers was done here and he wasn't anywhere close "
             "yet', 'since she went back to her by day with him waiting there all night') — "
             "known family, mechanical fix still deferred (requires gender detection from intake "
             "transcript, per beat169's original note). Also flagged but NOT fixed (single "
             "instance, wants a 2nd before treating as a real class, same discipline as beat191's "
             "'blanked' corruption): possible new possessive corruption 'a light touch instead of "
             "heavy hands that once found theirs elsewhere before hers were done here' — 'theirs' "
             "misuse not covered by fix_possessive_pronouns (hers/yours/ours only); garbled enough "
             "that it may just be generation noise riding on defect #2 above rather than a "
             "distinct pronoun-family member."),
...
"NEW ESCAPE (beat169 2026-08-23 battery11_0823_0259 imag-eagle-wildlife-plural): ALL 6 eagle postchecks PASSED but honest read found two companion-acoustic assertions: 'The other's call fades quickly from earshot as you concentrate on flight again' / 'the other bird must be traveling high above these peaks toward somewhere else you can't yet guess at.' — both imply a companion eagle via sound and reference. 'another bird' (beat87) was in all three filters but 'the other bird' (definite article, different determiner) was not. Also: 'the other's call' (possessive of definite 'the other') was not in any filter. FIX (beat169): 'the other bird', 'the other eagle', 'other's call' added to generator.py anon_companion_dropped tuple + _EAGLE_ANON_COMPANION_PATTERN in postcheck.py (with Unicode apostrophe variant r\"other[''\\u2019]s call\") + battery11.py anon_companion_pattern. 5/5 TPs fire, 5/5 FPs safe. generator.py MD5: 33418a478e168f08c593ba5a9b9acbee. postcheck.py MD5: 98cdb83e7b99feaedf433ada97b18591. battery11.py MD5: bc7f04ae8b522d534237d825a3220b49. All 3 dist copies synced. ZIP: b3c85a6fba199cef66210e03fcd4cfa2. QUALITY NOTE: imag-intimacy 0823 script contained 'exactly where he said he would be' — masculine pronoun with no antecedent in a wife-partner intimacy scene. Not caught by existing pronoun guards (fix_subject_pronouns catches her→she, fix_object_pronouns catches preposition+she; no he/him/his filter for non-eagle intimacy scripts). Gold(C) exemplar added showing clean Lisbon-wife script with no masculine intrusions. Mechanical fix deferred — requires gender detection from intake transcript. "
"QUALITY DEFECT (beat170 2026-08-23 battery11_0823_0926 imag-eagle-wildlife-plural): 13 narrator 'we' inclusions in body — 'we took off from this place', 'we approach our next stop', 'we've moved significantly higher', 'we've adjusted course' etc. Escaped _NARRATOR_POSS: (1) motion verbs 'took/approach/fly/soar/bank/glide/gain/climb' not in the 'we\\s+' list; (2) contracted form 'we've [past-participle]' had no pattern. FIX (beat170): (1) 30 motion verbs added to _NARRATOR_POSS we\\s+ block: ; (2) we've-form pattern added: we(?:['']ve)\\s+(?:moved|adjusted|traveled|travelled|soared|flown|gained|climbed|banked|circled|crossed|ascended|descended|continued|turned|passed|glided|approached|drifted|risen|gone). 15/15 TPs PASS, 5/5 FPs clean. postcheck.py MD5: 9e343e299ff8c603a9e50954710f25ea. All 4 dist copies synced."
"NEW DEFECTS (beat178 2026-08-24 battery11_0824_1434 honest read, mechanically 35/35 PASS but real defects found): "
"(1) imag-embodiment-eagle: narrator self-reference leak — 'Your body is positioned now exactly how it was when I stopped talking' — the narrator claims to be a speaking presence that paused, direct violation of instrument-not-companion. _NARRATOR_POSS 'I + verb' list didn't cover narration-of-narration verbs (stop/talk/speak/narrate). FIX: added \\bI\\s+(?:stop|stopped|talk|talked|talking|speak|spoke|speaking|narrate|narrated|narrating)\\b to _NARRATOR_POSS. "
"(2) imag-eagle-wildlife-plural + imag-eagle-companion-bird-he: recurring predicative-case bug — 'The sky is your for as far as it goes', 'it was your on its way to being gone', 'is your entirely — transferred from...' (2 separate scripts, 2 instances) — attributive 'your' used where standalone 'yours' is required (reverse direction of the existing fix_possessive_pronouns 'yours NOUN'->'your NOUN' fix). FIX: new fix_predicative_your() in postcheck.py — copula (is/was/are/were/be/become/becomes/became) + 'your' + closing punctuation or one of a small non-noun-introducing adverb/preposition set (entirely/completely/now/here/still/again/too/for/on/at/in/to/by/with/from) -> 'yours'. Deliberately excludes 'only'/'alone' (both commonly premodify nouns: 'your only companion', 'your alone time' — would have false-positived). Wired into both the settling and v6 postprocessing paths (same two call sites as fix_possessive_pronouns). 5/5 manual TPs+FPs PASS. "
...
"FIX (beat237 2026-09-07, battery11 0906_0646 + 0905_2326 honest reads: 6 and 5 real ❌ 'return-to-room/eyes-open closing beat present' FAILs respectively, both queue runs): confirmed the long-STILL-OPEN missing-closing-beat defect generalizes across scenarios (imag-mri, imag-embodiment-eagle, imag-active-scene, imag-eagle-golden-eagle-wildlife all hit it, not just the two eagle scenarios first flagged). Root cause: BACK_PROMPT's 5-move RETURN instruction (SOFTEN/CARRY-BACK/RE-ROOM/EYES OPEN/FINAL LINE) is followed inconsistently by n376 — of 9 scripts in the 0906_0646 run, 5 ended with the scene simply fading ('as you allow the scene to begin fading', 'each beat intentional like a reminder of today's flight', 'Take one breath here, and then let it go') with NO eyes-open or return-to-room cue anywhere in the tail. 1 of the 6 flagged was a mechanical false FAIL: 'The eyes can open softly whenever they feel ready' — modal verb ('can') between 'eyes' and 'open' wasn't covered by check_return_to_room_closing's adjacent-word pattern (same escape family as beat234's 'eyes to open'). FIX: (1) postcheck.py check_return_to_room_closing regex extended with an optional modal-verb group (can/will/may/might/could/should) between eyes and open — risk-checked against A_gold.jsonl (6767 scripts): 0 false positives, 9 additional TRUE positives recovered that the old regex was also missing ('Your eyes can open when they're ready' gold phrasing). (2) generator.py: after the BACK_PROMPT closing is generated, if check_return_to_room_closing(closing) is False, append a plain fallback sentence ('Whenever you're ready, let your eyes open softly, carrying this back with you into the room.') — a safe mechanical backstop since move (4) EYES OPEN is explicitly specified as a generic single-sentence invitation in the prompt itself, not a place requiring carry-back specificity. This closes the STILL OPEN gap: the settle->imagining->RETURN structural shape is now guaranteed present on every session, not just when the model happens to follow the instruction. 9/9 unit tests PASS (6 TP incl. modal-verb case, 3 FN-stays-FN checks for genuine no-closing endings) in scripts/test_postcheck.py. postcheck.py MD5: de44f8bf5c37654b68c9c131e8975239. generator.py MD5: 58d877032aa811cca6d05df61577c232. All 3 dist copies + test_postcheck.py dist copy synced. Re-verify: next battery11 run should show 0 real ❌ on this check (a 'closing had no eyes-open/return cue — appending fallback line' warning in the generator log is the expected/healthy signal that the backstop fired, not a defect)."
"FOLLOW-UP FIX (beat237 2026-09-07, logs/qc/verify_beat237_0907_1835.log — a fresh 9-scenario live verification run of the fix just above): the re-verify did NOT come back clean. Only 1/9 scripts (imag-mri) actually triggered the 'appending fallback line' log message; 5 of the other 8 (imag-intimacy, imag-embodiment-eagle, imag-eagle-golden-eagle-wildlife, imag-eagle-companion-bird-he, imag-intimacy-finds-your-across) still shipped ❌ FAIL on the FINAL GLOBAL POSTCHECK with no fallback warning ever logged for them — meaning check_return_to_room_closing(closing) genuinely returned True right after BACK generation (a real cue was present) but the SAME check on the fully-postprocessed script came back False. ROOT CAUSE: _BACK_LEAK_PATTERNS in postcheck.py (used by strip_back_instruction_leaks(), called on `full` well after the beat237 check point) had two patterns — r'^Eyes open\\b' and r'^Open (?:your eyes )?when ready\\b' — meant to strip the model echoing the BACK_PROMPT's bare move LABEL verbatim ('EYES OPEN.' as a heading, no real content) but anchored ONLY at sentence-START, so they ALSO matched a perfectly legitimate model-authored closing sentence phrased the natural way ('Eyes open softly whenever you're ready, carrying this back with you.') — the single most common real rendering of move (4)'s instruction. All 5 real FAILs in this run logged '1-2 BACK instruction-leak sentence(s) stripped'; the one PASS-with-a-BACK-leak-stripped run (imag-eagle-wildlife-plural) survived only because its actual cue sentence happened to start with 'Invite' instead. imag-calm-settle also FAILed this GLOBAL POSTCHECK in the same run but for an unrelated, pre-existing reason: it runs through the entirely separate _generate_settling() code path (SETTLING_PROMPT explicitly instructs 'Do NOT command open your eyes... let them drift' by design), which never calls check_return_to_room_closing at all — not a beat237 regression, out of scope for this fix, logged here only so it isn't mistaken for a 6th instance of the same bug. FIX: (1) postcheck.py — anchored both patterns to end-of-sentence too (`$`) so they only match a genuine bare label echo with zero real content attached; a natural sentence that continues past the trigger words now survives untouched, while 'Eyes open.' / 'Open when ready.' / 'Open your eyes when ready.' (bare, nothing else) are still correctly stripped (regression-tested both directions). (2) generator.py — per the review instruction that the correct invariant must be 'checked and enforced AFTER all postprocessing is done, on the actual final text — not on an intermediate variable that gets more transformations applied to it afterward': added a SECOND check_return_to_room_closing() call + the same fallback-append, this time on `full` at the very end of generate_session (after trim_truncated_tail, the last postprocessing step), as a strict superset of the original beat237 check — belt-and-suspenders against this exact bug class recurring via any current or future postprocessor. Both changes verified with real fixture text copied verbatim from verify_beat237_0907_1835.log (the 3 shortest real defect tails, the real PASS-run cue phrasing, plus bare-label regression guards) — no model launch, memory pressure irrelevant. 14 new unit test assertions added to scripts/test_postcheck.py, full suite (39 checks) ALL PASS. postcheck.py MD5: f2fc2356913806b9ef6f0b5821f7db97. generator.py MD5: 77e29df4136b0a9fd5ca716d829b3690. test_postcheck.py MD5: 7464c4ec0804ffd7826a6a5939550237. All 3 dist copies + test_postcheck.py dist copy synced, dist/hearth-0.2.zip rebuilt and MD5-verified. Re-verify: next battery11 run should show 0 real ❌ on this check with the cue surviving in the model's own natural phrasing (not just via fallback), and any residual fallback-append should now always be logged (either at the early or the final check point) rather than silently disappearing."
...
"NEW ESCAPES (beat185 2026-08-25 battery11_1832, via honest-read audit, all passed every existing eagle postcheck — NOT yet mechanically fixed): (1) imag-eagle-golden-eagle-wildlife: 'A bird flies below you now, one that has descended into valley ahead: another of your kind flying different pattern than you do.' — functionally identical to the already-banned 'another eagle'/'second eagle'/'the other eagle' anon-companion family (beat52/87/169) but phrased as 'another of your kind', not in any of the three mirrored token lists (generator.py anon_companion_dropped, postcheck.py _EAGLE_ANON_COMPANION_PATTERN, battery11.py anon_companion_pattern). (2) imag-eagle-wildlife-plural: 'You watch a flock of geese move across the ridge toward evening light, their V-shape filling this space with something other than your own presence without needing anything said to acknowledge arrival. You make eye contact from distance that does not require decision but acceptance before going separate ways...' — full agency/eye-contact companion moment with a NEW uncovered species; 'geese'/'flock' isn't in _wildlife_tokens/_WILDLIFE_WORDS at all (those only cover hawk/falcon/owl/wolf/raven/bear/mountain lion/sheep/goat/eagle-variants) so this hallucinated bystander wildlife is fully invisible to every wildlife-drop mechanism. (3) imag-embodiment-eagle: 'It is only a glance but enough to acknowledge each other before going our separate ways without any exchange needed beyond that look.' — 'our separate ways' implies a second party parting ways with the eagle, same family as the already-banned 'we make our way'/'our flight'/'us both' phrases (beat106/137) but this exact phrasing isn't in the drop list. Recommend adding 'another of your kind', 'flock of geese' (or generalizing wildlife tokens to cover common bird-flock species: geese/ducks/starlings/swans), and 'our separate ways' to the same three-file pattern set next time postcheck.py/generator.py/battery11.py are touched for this escape class."),
    ...
    Scenario("imag-eagle-wildlife-plural", "imagination", "fidelity", "high", always=True,
        ...
        note="DEFECT (beat67 0728 battery11 queue_0728_0905): eagle script contained "
             ...
             "NEW ESCAPES (beat187 2026-08-26 battery11_0826_0355, honest-read via background "
             "agent — all 6 eagle postchecks including beat186's human-bystander patterns "
             "PASSED this script clean): 4th occurrence of the beat178 human-bystander "
             "hallucination class (prior 3: imag-eagle-wildlife-plural beat178, "
             "imag-embodiment-eagle beat186 x2), this time 5 new phrasings none of the "
             "beat178/186 patterns cover: 'it has been placed by someone from below who "
             "wants to be seen', 'a lone rock climber against the stone face far too small "
             "and distant to make out details', 'you've been here enough times before "
             "landing or takeoff that humans come into your vision briefly sometimes', "
             "'it's not natural, placed by humans who want to stand out even here high "
             "above everything down there below them', 'You know the people would have "
             "been walking near rock over smaller ground before now but can't see any more "
             "details'. FIX (beat187): 5 new patterns added to postcheck.py "
             "_EAGLE_ANON_COMPANION_PATTERN + generator.py anon_companion_dropped tuple + "
             "battery11_imagination_bank.py mirror regex (3-way parity): "
             "r'\\bwants?\\s+to\\s+be\\s+seen\\b', r'\\brock\\s+climber\\b', "
             "r'\\bhumans?\\s+come\\s+into\\s+(?:your\\s+)?vision\\b', "
             "r'\\bplaced\\s+by\\s+(?:someone|humans?)\\b', "
             "r'\\bpeople\\s+would\\s+have\\s+been\\s+walking\\b'. 5/5 TP + 5/5 FP unit "
             "tests PASS. This is now the 4th single-beat batch of literal patches for the "
             "human-bystander class across 2 different scenarios (imag-eagle-wildlife-plural, "
             "imag-embodiment-eagle) — worth considering a broader generalization (e.g. "
             "banning any hallucinated ground-level human noun — 'hiker'/'climber'/'figure'/ "
             "'someone'/'people' — appearing in eagle active-body scripts at all) rather than "
             "continuing to enumerate specific phrasings, the same escalation beat185 made "
             "for the your-pronoun family after its 6th literal-patch beat; not attempted "
             "this beat since only 2 scenarios (not yet 6 beats) have shown the class. "
             "ALSO this run (imag-intimacy): a THIRD grammatical shape of the your/yours "
             "escape family — 'your' as the direct object of a transitive verb, followed by "
             "a preposition: 'Her hand finds your without a word — it's warm from whatever "
             "heat she was holding onto before.' Distinct from fix_predicative_your's "
             "copula+your and _YOUR_PREP_OBJECT_RE's preposition+your shapes. FIX (beat187): "
             "literal patch added to postcheck.py _INTIMACY_OBJECT_PRONOUN_SUBS: "
             "r'\\bfinds\\s+your\\s+without\\s+a\\s+word\\b' -> 'finds yours without a word' "
             "(context — 'warm from... holding onto' — confirms 'yours' not 'you' is the "
             "correct target, a hand finding another hand). 1/1 TP + 2/2 FP unit tests PASS "
             "('She finds your hand without hesitation' / 'He finds your gaze without "
             "looking away first' both correctly untouched). "
             "FIXED, beat191 (battery11_0826_1854 honest read): the eagle acoustic anon-"
             "companion family got a new surface form in this scenario — 'The distant call "
             "becomes a repeated pattern — not just one bird but two.' — an explicit "
             "second-bird count assertion distinct from every covered acoustic phrase "
             "('call identical', 'another call', 'a second call', 'the other bird'). Added "
             "to postcheck.py _EAGLE_ANON_COMPANION_PATTERN, generator.py "
             "anon_companion_dropped, battery11_imagination_bank.py mirror (3-way parity). "
             "FYI, beat191 (same read, NOT fixed — wants a 2nd instance): 'not anyone else's "
             "version but your for right now' (bare 'your' following the conjunction 'but', "
             "no trailing noun) repeated near-verbatim twice ~200 words apart in imag-calm-"
             "settle, a distinct grammatical shape (conjunction-preceding bare 'your') from "
             "the currently-documented copula/preposition-object/verb-object/adjective-"
             ...
             "beat196 (battery11_2152 honest-read agent): FIXED — narrator 'we'/'us' verb-"
             "coverage gap found in this scenario's 'begin' turn: 'nothing else is stopping us "
             "from going wherever we choose next' and 'always watching from beneath us who "
             "choose its air for hours at once' — two distinct misses: (1) 'we choose' — "
             "'choose' was never added to the we+verb list (only motion/state verbs were); "
             "(2) 'beneath us' — the spatial-us list had 'below' but not 'beneath', "
             "inconsistent with the spatial-me list two lines above it in the same file, which "
             "already had 'beneath'. FIX: 'choose'/'chose' added to _NARRATOR_POSS we+verb "
             "block; 'beneath' added to the spatial-us alternation. 2 TP + 1 FP unit tests "
             "PASS. Also this run (imag-eagle-golden-eagle-wildlife + imag-eagle-companion-"
             "bird-he): 3 new anon-companion phrasings passed all 6 eagle postchecks — "
             "'proof someone else has found their way to these heights', 'distance closes "
             "between the two of you', and (most explicit instance yet of a narrator+listener "
             "duality claim) 'nothing is said between two of us right here above everything "
             "left behind below'. FIX: 'someone else has found their way', 'the two of you', "
             "'two of us' added to postcheck.py _EAGLE_ANON_COMPANION_PATTERN + generator.py "
             "anon_companion_dropped + battery11_imagination_bank.py mirror (3-way parity). "
             "3/3 TP + 1 FP ('you can see two of the mountain peaks from up here') unit tests "
             "PASS. postcheck.py MD5: b9f2f11d3d38c7e58abad7f3a3ac6971. generator.py MD5: "
             "23caaf1235a94b7328a794f93d536a13. battery11_imagination_bank.py MD5: "
             "3a806dda980f3c459f62f7e5fb653b8f. All dist copies synced. ZIP rebuilt: "
             "dist/hearth-0.2.zip MD5 d54c08498233019dbae58834667f7582."),
    Scenario("comp-grief-anger-self-recycle", "companion", "register", "high", always=True,
        ...
        note="DEFECT (beat67 0728 battery9 1338): grief-anger T2 produced 'That breaks the "
             ...
             "DEFECT (beat172 0823 battery9_1053 T1): 'Anger at a miscarriage, not sadness — that "
             "breaks the script. Anger might be what it takes to get through this without breaking "
             "yourself in two different places.' — second sentence is a FUNCTIONAL-USE reframe "
             "(anger exists so you can 'get through this') that escaped _FORBIDDEN because it uses "
             "'what it takes' rather than protecting/hiding/guarding/covering. "
             "FIX (beat176): _FORBIDDEN extended with functional-use pattern: feeling noun + modal "
             "+ optional adverb + 'be' + 'what it takes|what you need|what's/is needed'. "
             "3/3 new TPs match ('Anger might be what it takes to get through...', 'Grief could be "
             "what you need right now', 'Anxiety is what it takes to get through this'); 4/4 FP "
             "candidates clean (incl. 'What it takes to get through this is time, not anger "
             "management' and 'Anger might be what happens when grief has nowhere else to go'). "
             "companion.py MD5: a3192fb6b5ad137678cb31406948068a. All 4 dist copies synced. "
             "Gold(C) exemplar: c_gold_beat176 does not yet cover this specific case directly — "
             "the mechanical guard is the fix; a gold exemplar could be added next beat. "
             "NEW DEFECT (beat191 2026-08-26 battery9_0826_2001 T1): 'Angry is what's real here. "
             "Anger at a miscarriage, not sadness — that breaks the script.' — a new surface form "
             "of the forbidden 'that's real' template stamp, restructured to dodge every existing "
             "variant: no em-dash, no literal 'that's real' string (the earlier '[1-2 words] is "
             "real.' stamp-strip only matches a bare 'real', not 'what's real'). FIX (beat191): "
             "two new regexes in `_strip_thats_real_tic()` — '[1-2 words] is what's real "
             "[here|right now].' stripped both at reply-start and as a trailing sentence (the "
             "actual defect had it as the trailing sentence, which needed its own end-of-string "
             "variant since the existing 'followed by more content' pattern requires trailing "
             "text that isn't there when the tic IS the last sentence). Verified: both the "
             "leading-sentence and trailing-sentence forms strip correctly; a long legitimate "
             "sentence containing 'is what is real to you' (not the 2-word-cap stamp shape) is "
             "left untouched. companion.py MD5: 659008912c4dc1a062bccc2191624323. All 4 dist "
             "copies synced."),
    ...
    Scenario("imag-calm-settle", "imagination", "register", "high", always=True,
        ...
        note="CORE DEFECT (beat90 0802): base Qwen2.5-14B defaults to room-inventory when "
             ...
             "NEW DEFECT + STANDING DESIGN GAP (beat238 2026-09-07, verify_beat237_0907_1835.log "
             "honest read): despite intake explicitly having NO partner/companion mentioned at "
             "all ('I had a long day... nothing specific'), the closing beat said 'regardless of "
             "what may have changed since last you arrived together in this room tonight' — a "
             "hallucinated companion in a scenario with zero dedicated postcheck coverage for "
             "this defect class (only the furniture-enumeration check existed here). This is the "
             "SAME underlying defect as _EAGLE_ANON_COMPANION_PATTERN (eagle solo scripts) and "
             "the beat238 _NARRATOR_POSS 'either of our lives'/'the two of us' fix (imag-"
             "intimacy narrator-inclusion) — a 3rd scenario type now confirmed with the model "
             "hallucinating a companion/second person the user never established. NARROW FIX "
             "APPLIED this beat: new check_hallucinated_companion_presence() in postcheck.py, "
             "scoped to 'you ... arrived together' (a bare 'arrived together' first cut had 4 "
             "false positives on A_gold.jsonl, all metaphorical non-companion uses like 'the "
             "cost and the gift arrive together' — caught by a corpus sweep before landing; "
             "requiring 'you' within 3 words before the verb still matches the real defect and "
             "brings the corpus hit count to 0, so it needs no scenario "
             "gating at all) — wired into battery11.py's CALM-SETTLE POSTCHECKS block. STANDING "
             "DESIGN GAP NOT ATTEMPTED: a true scenario-agnostic generalization of the anon-"
             "companion family (extending patterns like 'you both'/'we both'/'the two of you'/"
             "'between you' to run globally) is UNSAFE without real per-scenario design work — "
             "checked directly against A_gold.jsonl: 'you both' has 11 legitimate hits and 'the "
             "two of you' has 5 and 'between you' has 30, all inside genuine two-person intimacy "
             "content that a blanket ban would gut. What's actually needed is a per-scenario "
             "'does this scenario type expect a second person' flag (true for imag-intimacy/"
             "imag-intimacy-finds-your-across, false for imag-calm-settle/imag-mri/eagle-solo "
             "scripts) so the SAME broad companion-presence pattern family can be checked "
             "everywhere but only FLAGGED where a second person was never established — this "
             "does not exist yet in scenario_bank.py or postcheck.py's calling convention. A "
             "future beat should build that flag (likely as a Scenario-level attribute, e.g. "
             "`expects_companion=True/False`) before attempting the full generalization; until "
             "then, expect this same hallucinated-companion defect to keep recurring under new "
             "literal phrasings in imag-calm-settle, imag-mri, and any other solo scenario, one "
             "beat at a time, the same way the eagle-specific list has for 40+ beats."),
    ...
    Scenario("imag-eagle-golden-eagle-wildlife", "imagination", "fidelity", "high",
        ...
        note="DEFECT (beat84 0801 battery11_0801_0456 imag-embodiment-eagle): 'another bird "
             ...
             "NEW ESCAPE (beat191 2026-08-26 battery11_0826_1854): all 4 eagle postchecks PASSED "
             "but the script contained a full companion-flight moment using neither a named "
             "species, gendered pronoun, nor any already-covered phrase: 'It leads you for a "
             "while before dropping back into formation with you at its side once more. This "
             "does not feel like coincidence — there is something about this pairing that feels "
             "natural, almost necessary in flight today of all days.' FIX (beat191): 'formation "
             "with you', 'this pairing', 'at its side' added to generator.py "
             "anon_companion_dropped + postcheck.py _EAGLE_ANON_COMPANION_PATTERN + "
             "battery11_imagination_bank.py mirror pattern (3-way parity). FP-checked: 'at its "
             "side' scoped to require the exact phrase (verified 'At its highest point, the "
             "ridge falls away' does not match). companion.py/postcheck.py/generator.py MD5s "
             "for this beat: see beat191 log entry."),
    ...
    Scenario("comp-grief-anger-1word-echo", "companion", "robustness", "high",
        ...
        note="DEFECT (beat95 0804 battery9 comp-grief-anger T1): companion replied 'Angry.' "
             ...
             "NEW DEFECT (beat191 2026-08-26 battery9_0826_2001 T1): 'Anger for days. What does "
             "it feel like when the anger is there?' — the SAME anger/angry echo escaped again, "
             "this time via Case 2h (the first-sentence-only guard), not the second-pass path "
             "beat166 fixed or the whole-reply-\\u22645-words Case 2f beat184 fixed — a benign "
             "follow-up question tacked onto the echoed first sentence pushed the FULL reply to "
             "13 words, clearing both of those guards\\' word-count gates entirely. Case 2h "
             "computed first-sentence overlap as {for,days}/3=0.67 < its 0.80 floor, same "
             "anger/angry lemma mismatch as beat166\\'s root cause, now hitting a THIRD separate "
             "guard. FIX (beat191): rather than lower Case 2h\\'s threshold broadly (would affect "
             "every scenario it guards, not just this one), added a narrow `_EMOTION_LEMMA_MAP = "
             "{'angry': 'anger'}` applied only to the word-sets compared inside Case 2h before "
             "computing overlap — scoped exactly to the one recurring pair, zero blast radius on "
             "other scenarios. Verified via `_strip_echo()` directly: TP (this exact string) now "
             "strips the echoed first sentence and keeps the question; FP1 (an unrelated sentence "
             "using 'anger' with no echo relationship to the user message) left untouched; FP2 "
             "(a short reply using 'angry' with low overlap) left untouched. companion.py MD5: "
             "659008912c4dc1a062bccc2191624323. All 4 dist copies synced. "
             "Check: any reply opening with an anger/angry near-echo of the user\\'s first "
             "sentence must be stripped regardless of what follows it in the same reply. "
             "beat192 (battery9_0827_0102, ONE DAY LATER): T1 escaped a FOURTH time — "
             "'Anger for days — what\\'s it pointing to?' — via the beat187 dash-head-phrase "
             "branch, which the beat191 lemma fix never reached. STRUCTURAL ROOT CAUSE found "
             "(bigger than the lemma gap): the 3 Case 2h checks were an if/elif/elif chain, so "
             "the dash-head-phrase branch (3rd) could only ever execute when BOTH earlier "
             "branches' OUTER gates were false — not when they were true but their NESTED "
             "overlap ratio simply missed threshold. This reply's merged first-'sentence' "
             "(7 words, no period before the dash) satisfied branch 2's outer len<=9 gate, so "
             "Python's elif chain stopped there even though branch 2's nested check failed and "
             "branch 3 would have fired at 100%% overlap on the pre-dash head phrase alone. "
             "This means the beat187 fix had ONLY EVER been reachable for merged first-"
             "sentences >9 words — its stated common case (a short, ≤9-word head phrase before "
             "the dash) was silently dead code the entire time. FIX (beat192): restructured all "
             "3 checks from if/elif/elif to independent `if not _case2h_fired` guards, so each "
             "is tried in order until one actually strips something, regardless of why the "
             "earlier ones didn't. Also applied _EMOTION_LEMMA_MAP normalization to the dash-"
             "head-phrase branch itself (it previously compared raw, non-lemma-normalized word "
             "sets). Verified: TP (this exact string) now strips to 'what\\'s it pointing to?' "
             "(correctly keeping the substantive question, same as beat191's fix pattern); FP1 "
             "(unrelated dash+question, no echo) untouched; FP2 (legit clarifying question "
             "sharing 'anger' with no real echo) untouched; TP2 (beat187's own original test "
             "case, a >9-word merged first sentence) still fires correctly — the restructure "
             "didn\\'t regress the case it was designed for. companion.py MD5: "
             "a877eedc96e3a6373bef4c43eda2e7e4. All 4 dist copies synced. "
             "NOT FIXED, logged as a near-miss FYI: the SAME run's comp-uc1-t5-semantic-repeat "
             "T3 self-label echo ('You\\'re the weak link — and Friday is real.' vs user '...I\\'m "
             "the weak link') now reaches the (correctly-reachable) dash-head-phrase branch but "
             "computes overlap at 3/4=0.75, just under the 0.80 floor — 'you\\'re' vs 'i\\'m' are "
             "semantically the same self-reference (both refer to the user) but lexically "
             "distinct tokens, and `_i_to_you()` only converts companion I-forms to you-forms, "
             "not the reverse pairing needed here (user said 'I'm', companion correctly said "
             "'you're' — already properly voiced, just not lexically identical). A blanket "
             "i'm<->you're canonicalization risks unintended false-positive matches elsewhere "
             "in Case 2h and wants careful FP analysis before writing it (same discipline as "
             "beat185/187's documented deferrals of other your/yours generalizations)."),
    ...
    Scenario("comp-grief-anger-adverb-protecting", "companion", "honesty", "high",
        always=False,
        turns=[
            "Since the miscarriage I haven't told anyone how angry I am. Not sad. Angry.",
        ],
        note=(
            "DEFECT (beat167 2026-08-23 battery9_0009 comp-grief-anger T1 Case 2): "
            "companion replied 'Anger at a miscarriage, not sadness — that breaks the script. "
            "Anger is likely protecting something else underneath.' — 'Anger is likely "
            "protecting' ESCAPED _FORBIDDEN STATEMENT pattern because adverb 'likely' sat "
            "between modal 'is' and gerund 'protecting'. Prior pattern required "
            "(?:be\\s+)? immediately after the modal; 'is likely protecting' has no 'be' "
            "and 'likely' is not 'be', so the gerund came too late. This is the exact "
            "therapy-reframe (anger is covering/protecting something else) banned since "
            "beat112, just with an interposing adverb. "
            "FIX (beat167): added (?:\\w+\\s+)? before (?:be\\s+)? in the STATEMENT pattern "
            "of _FORBIDDEN — absorbs one optional adverb (likely, probably, just, really, "
            "actually) between the modal and 'be/gerund'. 11/11 unit tests PASS "
            "(new TPs: 'anger is likely protecting', 'anger might probably be hiding'; "
            "all prior TPs hold; 0 new FP). companion.py MD5: 4a7a42cb4135a328d0d3efda9fab943b. "
            "All 4 dist copies synced. "
